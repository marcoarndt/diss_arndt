...
Für derartige Selektionsaufgaben eignen sich \textbf{Screening-Versuchspläne}, wie der \textbf{teil-faktorielle Versuchsplan} (Fractional Factorial Design) oder alternativ der \textbf{Plackett-Burman-Plan} \cite{Kleppmann.2016,Siebertz.2017,Montgomery.2020}.
Bei diesem Ansatz wird lediglich eine Fraktion der voll-faktoriellen Versuchsagenda umgesetzt. Mathematisch wird die Anzahl der Versuche dabei auf $\sym{n} = 2^{\sym{k}-\sym{p}}$ reduziert, wobei $\sym{p}$ den Grad der Fraktionierung angibt.
...
\end{itemize}
Eine direkte Konsequenz dieser Reduktion ist jedoch, dass sich in teil-faktoriellen Versuchsplänen bestimmte Effekte nicht mehr isoliert betrachten lassen (vgl. Abbildung~\ref{fig:abb2.05.1_vfdesign_plot} für den Fall einer Fraktionierung).
