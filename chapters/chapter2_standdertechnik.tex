...
Die Anwendung von \ac{DoE} versteht sich grundsätzlich als Verfahrenskette \cite{Coleman.1993,Montgomery.2020} entlang mehrerer Prozessschritte, die beginnend von einer spezifischen Aufgabendefinition in einer statistisch abgesicherten Testentscheidung und Datenmodellierung mündet, vgl. Abbildung~\ref{fig:ma_abb2.02_doe_steps}.
\begin{figure}[htbp]
    \centering
...
\end{equation}
Ein Modell zweiter Ordnung enthält zudem quadratische Terme, welche üblicherweise in Optimierungsaufgaben relevant werden - so auch in der Lebensdauer- und Zuverlässigkeitsanalyse:
...
Optimale Versuchspläne (Computer-Generated Designs) erweitern das methodische Spektrum, indem sie eine maximale Flexibilität jenseits starrer Standard-Designs ermöglichen.
Ihr Einsatz ist prädestiniert für Szenarien, in denen klassische orthogonale Pläne an ihre Grenzen stoßen - sei es durch physikalische Randbedingungen welche bestimmte Faktorstufenkombinationen ausschließen, oder durch strikte Limitierungen der verfügbaren Versuchskapazität.
Ein Versuchsplan wird hier als \textbf{orthogonal} bezeichnet, wenn keine Korrelation zwischen jeweils zwei Spalten der Versuchsplan-Matrix vorliegt - deren Skalarprodukt also null ergibt:
\begin{equation}
...
Im Gegensatz zu Standardplänen erlauben optimale Designs beliebige Freiheitsgrade hinsichtlich der Stichprobengröße und der Modellkomplexität.
