...
Die Ausfallzeit wird als kontinuierliche Zufallsvariable $\sym{tau}>0$ verstanden.
...
\begin{longtable}{@{} p{\textwidth} @{}}
    ...
    \textbf{Wahrscheinlichkeitsdichte} (engl. \textit{\ac{pdf}}): \newline
    ...
    \textbf{Kummulative Ausfallwahrscheinlichkeit} (engl. \textit{\ac{cdf}}): \newline
...
Tabelle~\ref{tab:deskriptive_statistik} fasst die theoretischen Definitionen der Zufallsvariable $\sym{tau}$ sowie die grundlegenden Zuverlässigkeitsfunktionen und ihre korrespondierenden empirischen Schätzer kompakt zusammen.
    {
        ...
        \begin{longtable}{@{} p{\textwidth} @{}}
            ...
            Erwartungswert $\sym{mu}$ des theoretischen Lageparameters der Grundgesamtheit. Der arithmetische Mittelwert $\sym{x_bar}$ dient als üblicher, \textit{empirischer Schätzer} für eine endliche Stichprobe.
...
Je nach zugrundeliegendem physikalischen Ausfallmechanismus finden jedoch auch andere statistische Verteilungen Anwendung, wie beispielsweise die \textit{Lognormal-Verteilung} (häufig bei Ermüdungs-, Korrosions- oder Diffusionsprozessen) oder die \textit{Exponentialverteilung} (zur Modellierung von Zufallsausfällen ohne Alterungseffekte) \deleted[id=Daz]{oder die \textit{Beta-Verteilung} (allgemein zur formenreichen Modellierung von $\sym{R}$ über dem festen Intervall $[0,1]$)} \cite{Yang.2007,Birolini.2017}.
...
Die Flexibilität der Verteilung zeigt sich in den Spezialfällen des Formparameters: Für $\sym{b}=1$ geht sie in die Exponentialverteilung über ($\sym{lambda} = \text{konst.}$), während sie sich für $\sym{b} \approx 3,6$ \highlight[id=Daz, comment={b=3,60235 -> Schiefe=0}]{einer Normalverteilung} annähert (vgl. Abb.~\ref{fig:abb2.1_weibull}) \cite{Rinne.2008, Kececioglu.2002}.
\begin{figure}[htbp]
    \centering
...
Das universell anwendbare und robuste Verfahren, das Herausforderungen wie zensierte Daten und multivariate Modelle inhärent behandelt, ist die \textit{\ac{MLE}} \cite{Meeker.2022,Nelson.1990}.
...
Da Lebensdauerparameter (z.\,B. $\sym{T}, \sym{b}$) definitionsgemäß auf $\sym{RR}_{>0}$ beschränkt sind, erfolgt die Berechnung der \acp{CI} standardmäßig über eine Log-Transformation der Parameter.
...
Dementsprechend entscheidend ist das Verständnis der wesentlichen Grundlagen im Umgang mit statistischer Versuchsplanung (\ac{DoE}) für Lebensdauerdaten - nach \textcite{Kremer.2021} auch unter dem Begriff \textit{\ac{L-DoE}} zusammengefasst - sowie der darauffolgenden Lebensdauermodellbildung.
...
Hier wird in der industriellen Praxis häufig fälschlicherweise ein \textbf{\ac{OFAT}}-Testing Ansatz gewählt - unabhängig, ob vom Vorhandensein von Lebensdauer- oder anderen Daten, welcher schlichtweg die Wahrscheinlichkeit, Optimalstellen im Parameterraum systematisch zu treffen, senkt und somit gegenüber \ac{L-DoE} nachteilig ist, vgl. \cite{Montgomery.2020,Siebertz.2026}.
...
Für eine grundsätzlichere Auseinandersetzung mit konventionellen Methoden und Werkzeugen von \ac{DoE} sei, mit Blick auf den Fokus der vorliegenden Arbeit, hingegen auf die einschlägige Literatur von \textcite{Kleppmann.2020}, \textcite{Siebertz.2026}, \textcite{Hinkelmann.2012} sowie vornehmlich \textcite{Montgomery.2020} und \textcite{Myers.2016} verwiesen.
...
Multivariate Lebensdauertests erfordern definitionsgemäß die Variation mehrerer Einflussfaktoren ($\sym{k} \geq 2$).
...
Während detaillierte historische Abrisse zur klassischen Versuchsplanung der einschlägigen Standardliteratur zu entnehmen sind \cite{Fisher.1935, Box.1978, Montgomery.2020}, fokussiert dieser Abschnitt explizit auf die methodischen Anforderungen mehrdimensionaler Lebensdauertests.

Der primäre Anspruch von \ac{DoE} besteht in der ökonomisch effizienten empirischen Modellierung einer Systemantwort in Abhängigkeit erklärender Variablen.
In der Zuverlässigkeitstechnik zielt dies auf die parametrische Charakterisierung des multivariaten Ausfallverhaltens ab.
...
Diese räumliche Diskrepanz erzwingt eine formale stochastische Extrapolation.
...
Dadurch bleibt die tatsächliche prädiktive Vorhersagegüte und geometrische Effizienz der Versuchspläne im Feldbetrieb oft unberücksichtigt.

Um diese Herausforderung zu bewältigen, sind empirische Best-Guess-Ansätze oder die in der industriellen Praxis noch häufig anzutreffende \ac{OFAT}-Methodik (\textit{One Factor At a Time}) unzureichend.
Sie binden experimentelle Ressourcen ineffizient und sind systembedingt nicht in der Lage, multivariate Wechselwirkungen aufzulösen \cite{Kleppmann.2020,Siebertz.2026}.
Da die topologische Struktur eines Versuchsplans die erreichbare Modellierungs- und Extrapolationsgüte deterministisch begrenzt, ist eine präzise mathematische Bewertung der Plangüte vor der Versuchsdurchführung damit unerlässlich.
Hierfür dienen etablierte statistische Metriken wie die Prädiktionsvarianz sowie formale Optimalitätskriterien \cite{Montgomery.2020, Goos.2011}.

Vor diesem Hintergrund strukturiert sich der weitere Verlauf dieses Kapitels wie folgt:
Zunächst werden in Abschnitt~\ref{subsec:begriffedoe} die relevanten Grundbegriffe und Bewertungsmetriken zur Qualifizierung multivariater Versuchspläne eingeführt.
Anschließend erfolgt in Abschnitt~\ref{subsec:pläne} eine Übersicht der für Lebensdauertests geeigneten \ac{DoE}-Strukturen, bevor abschließend in Abschnitt~\ref{subsec:modellbildung} die verallgemeinerte statistische Modellbildung (Weibull-\ac{GLM}) beleuchtet wird.
Fundamentales Grundlagenwissen zur klassischen Regressionsmodellierung (\ac{ANOVA}), zum Umgang mit unzensierten normalverteilten Daten sowie die detaillierte mathematische Widerlegung der \ac{OFAT}-Methodik werden im Folgenden als bekannt vorausgesetzt.
Für diese Themenkomplexe sei auf Standardwerke von \textcite{Montgomery.2020}, \textcite{Kleppmann.2020} sowie themenverwandte Vorarbeiten am \ac{IMA} \cite{Dazer.2019, Herzig.2021} verwiesen.
...
Im Zentrum der Betrachtung steht damit generell ein technisches \textbf{System}, welches abstrakt als Produkt oder Prozess verstanden wird und den Zustand der Ausgangsgröße in Abhängigkeit der einwirkenden Eingangsgrößen definiert.
...
Die planerische Kombination verschiedener Faktorstufen äußert sich in spezifischen \textbf{Versuchspunkten} innerhalb des Parameterraums und entspricht der Versuchsplan-Matrix \cite{Kleppmann.2020,Siebertz.2026}.
...
\end{equation}
So können mittels \ac{DoE} strukturiert, effizient und verbindlich Informationen gewonnen werden, die über die direkten Effekte hinausgehen und differenziert Aufschluss über \textbf{Haupteffekte} sowie etwaige \textbf{Wechselwirkungen} der Faktoren auf die Antwort des Systems geben - vergleiche Abb.~\ref{fig:ma_abb2.04_effekt} sowie \textcite{Montgomery.2020,Kleppmann.2020,Siebertz.2026,Kremer.2021}.
...
Die Differenz der gewählten Level muss bereits im Vorfeld definiert werden, sodass signifikante Effekte sicher detektiert werden - vgl. \textbf{\ac{SNR}} \cite{Montgomery.2021} - wobei gleichzeitig zu geringe Abstände (Rauschen) sowie zu große Intervalle (Gefahr unerkannter Nichtlinearitäten) zu vermeiden sind, siehe auch Abbildung~\ref{fig:ma_abb2.06_effektabstand} sowie \textcite{Wu.2021,Siebertz.2026,Kleppmann.2020}.
\begin{figure}[htbp]
    \centering
...
Für derartige Selektionsaufgaben eignen sich daher \textbf{Screening-Versuchspläne}, wie der \textbf{teil-faktorielle Versuchsplan} (Fractional Factorial Design) oder alternativ der \textbf{Plackett-Burman-Plan} \cite{Kleppmann.2020,Siebertz.2026,Montgomery.2020}.
...
Der erste Schritt zur Detektion von Nichtlinearitäten besteht in der Integration von $\symsub{n}{idx_C}$ sogenannten \textbf{\acp{ZP}}, engl. \textbf{\acp{CP}}, in den faktoriellen Basisplan. Hierbei werden alle Faktoren auf die kodierte Stufe \textit{0} (die Mitte des Versuchsraums) gesetzt.
...
\begin{itemize}
    ...
    \item $\symsub{n}{idx_S} = 2 \cdot \sym{k}$ zusätzliche \textbf{\acp{SP}} (engl. \textbf{Axial Points} oder \textbf{Star Points}), die auf den Achsen des Koordinatensystems im Abstand $\pm \symsub{alpha}{idx_D}$ vom Zentrum liegen.
\end{itemize}
\begin{figure}[htbp]
    \centering
    \begin{subfigure}[b]{0.4\textwidth}
...
Betrachtet man diesen Raum als Kugel (\textbf{sphärisch}), ist die \textbf{Drehbarkeit} (\textbf{Rotierbarkeit}, engl. \textbf{Rotatability}) ein wesentliches Qualitätsmerkmal.
...
\end{equation}
Alternativ kann für sphärische Räume auch $\symsub{alpha}{idx_D} = \sqrt{\sym{k}}$ gewählt werden (\textbf{Sphärisches \ac{CCD}}), wodurch alle Versuchs- und \acp{SP} auf einer Kugeloberfläche liegen \cite{Myers.2016}.
...
Im Sinne der absoluten Schätzpräzision und direkt über die Breite der resultierenden Konfidenzintervalle kann sie im ingenieurwissenschaftlichen Blickwinkel aber auch unskaliert betrachtet werden: als \textbf{\ac{UPV}}
\begin{equation}
    \sym{pred_var}/
...
Sie bewertet den Informationsgewinn pro investiertem Versuchsaufwand $\sym{N}$. Ein geometrisch ineffizient platzierter Versuchspunkt (etwa ein redundanter Zentralpunkt für eine fernen Extrapolation) kann zwar die absolute \ac{UPV} marginal senken, jedoch die \ac{SPV} verschlechtern, da die Aufwandserhöhung (Steigerung von $\sym{N}$) den minimalen Informationsgewinn überkompensiert.
Somit dient die \ac{SPV} folglich dem topologischen Strukturvergleich und der Budget-Allokation, während die \ac{UPV} die Basis zur Berechnung realer Konfidenzintervalle für die tatsächliche Lebensdauerprognose im Feld bildet.
...
Während der $\sym{A2}$-Indikator die Gesamtmatrix bewertet, stehen genau diese Diagonalelemente in direktem Zusammenhang mit dem \textbf{\ac{VIF}} für die Evaluation einzelner Modellterme \cite{Siebertz.2026,Kleppmann.2020}.
...
Zur lokalen Diagnose von Versuchspunkten und deren Auswirkung auf die Regressionsparameter existieren in der Fachliteratur etablierte Maßzahlen wie die Hebelwerte $\sym{h}_{\sym{idx_i}\sym{idx_i}}$ der Prädiktionsmatrix (engl. auch \textbf{Hat-Matrix}) $\sym{Hat} = \sym{X}(\sym{X}'\sym{X})^{-1}\sym{X}'$, die Cook-Distanz ($\symsub{D}{idx_i}$) sowie detaillierte Einflussstatistiken wie $\sym{dfbetas}$, $\sym{dffits}$ und $\sym{covratio}$ - vergleiche Werke von \textcite{Siebertz.2026, Montgomery.2021, Belsley.2004, Fahrmeir.2009}.
...
% Punkte mit hohen Hebelwerten (typischerweise $\sym{h}_{\sym{idx_i}\sym{idx_i}}> 2\sym{p}/\sym{n}$) befinden sich geometrisch weit vom Zentrum des Versuchsraums entfernt und dominieren die Regression, was den Plan anfällig für Ausreißer in diesen spezifischen Einstellungen macht \cite{Siebertz.2026,Montgomery.2021}.
...
% Sie verknüpft die Information des intern studentisierten Residuums $\symsub{r}{idx_i}$ mit dem Hebelwert $\sym{h}_{\sym{idx_i}\sym{idx_i}}$:
% % ...
% \begin{equation}
%     \symsub{D}{idx_i} = \frac{\sym{r}_{\sym{idx_i}}^2}{\sym{p}} \cdot \frac{\sym{h}_{\sym{idx_i}\sym{idx_i}}}{1-\sym{h}_{\sym{idx_i}\sym{idx_i}}}.
%     \label{eq:cooks_d}
% \end{equation}
% % ...
...
\begin{equation} \label{eq:fim_o_glm}
    ...
    \end{bmatrix}.
\end{equation}
Im Gegensatz zu klassischen linearen Modellen mit normalverteiltem Fehler hängt die Informationsmatrix im \ac{GLM}-Framework somit nicht ausschließlich von der geometrischen Versuchsmatrix $\sym{X}$ ab, sondern ist eine explizite Funktion des zu schätzenden Parametervektors $\sym{theta}$.
