...
Der Ausfall markiert das Lebensdauerende (\ac{EoL}), definiert durch den Verlust der geforderten Funktionalität unter spezifischen Randbedingungen. Maßgeblich hierfür sind äußere Belastungen (z.,B. Kräfte) und die resultierenden inneren Beanspruchungen (Spannungen). Die Ausfallzeit wird als kontinuierliche Zufallsvariable $\sym{tau}>0$ modelliert, deren Verteilungsfunktion $\sym{F}(\sym{t})=\sym{Pr}(\sym{tau}\leq \sym{t})$ die Ausfallwahrscheinlichkeit beschreibt. Komplementär dazu quantifiziert die Zuverlässigkeit $\sym{R}(\sym{t}) = 1 - \sym{F}(\sym{t})$ die Überlebenswahrscheinlichkeit eines nicht reparierbaren Systems \cite{Bertsche.2022}.
