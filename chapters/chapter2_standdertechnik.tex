...
Intuitiv erfordert bereits die Bestimmung der Parameter einer einfachen Geraden ($\symsub{beta}{idx_0}, \sym{beta}_1$) mindestens zwei unterschiedliche Stützstellen.
...
Im Kontext optimaler Versuchspläne definiert nach \textcite{Pukelsheim.2006} der Maßerweiterungssatz von Carathéodory dabei eine theoretische Obergrenze für die notwendigen Stützstellen von $\sym{p}(\sym{p} + 1)/2 + 1$, beziehungsweise $\sym{p}(\sym{p} + 1)/2$, sofern das Interesse der Schätzung aller Elemente des Parametervektors $\sym{Beta}$ gilt \cite{Russell.2019}.
