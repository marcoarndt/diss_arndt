...
Dementsprechend entscheidend ist das Verständnis der wesentlichen Grundlagen im Umgang mit statistischer Versuchsplanung (\ac{DoE}) für Lebensdauerdaten, auch \ac{L-DoE} sowie der Lebensdauermodellbildung.
Da die Struktur des Versuchsplans die erreichbare Modellierungsqualität der Zielgröße maßgeblich determiniert, ist neben dem Verständnis fundamentaler Zusammenhänge insbesondere die quantitative Bewertung von Gütekriterien der Versuchspläne essenziell.
Dazu dienen spezifische Performance-Indikatoren sowie sogenannte Optimalitäten, vgl. \cite{Montgomery.2020,Goos.2011}.
Aufgrund dessen beschäftigt sich der Inhalt dieses Abschnitts mit einer gezielten Auswahl an Grundbegriffen und Metriken für multivariate Testpläne im Zusammenhang mit Lebensdauerdaten sowie einer Übersicht der für Lebensdauertests geeigneten Versuchspläne, bevor abschließend die statistische Modellbildung beleuchtet wird.
...
Diese behandeln intensiv die Inhalte konventioneller statistischer Versuchsplanung, welche um Perspektiven zu Lebensdaueruntersuchungen bereits durch am \ac{IMA} entstandenen Dissertationen von \textcite{Dazer.2019}, \textcite{Herzig.2021}, \textcite{Kremer.2021} und \textcite{Grundler.2024} fortschreitend ergänzt wird - um nur eine Auswahl relevanter Referenzen zu nennen.
Konsequenterweise wird der Umgang mit normalverteilten Daten im Rahmen des \ac{DoE}, mit Regressionsmodellierung auf Basis der \ac{ANOVA}, mit konventionellen Hypothesentests und deren Hintergründe sowie mit fundamentalen Ausführungen zu \ac{ALT} daher innerhalb der nachfolgenden Ausführungen nicht explizit herausgestellt.

\subsection{Grundbegriffe der statistischen Versuchsplanung} \label{subsec:begriffedoe}
Die Anwendung von \ac{DoE} versteht sich grundsätzlich als Verfahrenskette \cite{Coleman.1993,Montgomery.2020} entlang mehrere Prozessschritte, die ausgehend von einer spezifischen Aufgabendefinition beginnt und in einer statistischen abgesicherten Lagebeurteilung mündet, vgl. \colorbox{yellow}{Abbildung~Doe-Steps}.
...
So kann dieser Zusammenhang - beispielhaft die zufallsverteilte Lebensdauer in Abhängigkeit $k>2$ technischer Beanspruchungen - modelliert und gegebenenfalls optimiert werden, vergleiche \textit{Schritt~1} in \colorbox{yellow}{Abbildung~Doe-Steps}.
...
Die planerische Kombination verschiedener Faktorstufen äußert sich dann in spezifischen \textbf{Versuchspunkten} innerhalb des \textbf{Parameterraums}.
...
Die hierbei erreichte Akquisition einer empirischen Datengrundlage folgt als entlang eines präzise beschriebenen Prozesses und innerhalb der Vorgaben vorab definierter Versuchspläne.
Das ergibt entscheidende Vorteile, wird der planerische Aspekt in die Ausführung aufgenommen.
Entgegen in der Praxis üblicherweise noch verbreiteter alternativer Vorgehen wie \ac{OFAT}-Testing können mittels \ac{L-DoE} effizient und verbindlich Informationen gewonnen werden, die über die faktorbedingten Effekte hinaus gehen und differenziert Aufschluss über \textbf{Haupteffekte} und etwaige \textbf{Wechselwirkungen} der Faktoren geben können.
Beide können gemäß beliebiger Polynomfunktionen höhere Ordnungen annehmen, als \textbf{quadratische Effekte} oder \textbf{Mehrfachwechselwirkungen} abbilden.
