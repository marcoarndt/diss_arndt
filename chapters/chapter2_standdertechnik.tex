...
Die statistische Modellbildung transformiert die durch den Versuchsplan generierte Datenbasis in einen funktionalen Zusammenhang zwischen den Einflussfaktoren $\sym{x}$ und der Lebensdauerantwort.
...
Stattdessen kommt das Framework der \textbf{Generalisierten Linearen Modelle} (\ac{GLM}) zur Anwendung, welches die lineare Prädiktion mit der zugrundeliegenden Weibull-Verteilung verknüpft \cite{Myers.2010,Rigdon.2022}.

Der Prozess der Modellierung gliedert sich hierbei in die folgenden methodischen Schritte:
...
\begin{equation} \label{eq:glm_link}
    \ln(\sym{T}) = \symsub{beta}{idx_0} + \sum_{\sym{idx_j}=1}^{\sym{k}} \symsub{beta}{idx_j} \symsub{x}{idx_j} + \dots + \sym{epsilon}.
\end{equation}
