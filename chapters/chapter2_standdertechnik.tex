...
Dazu wird die Stichprobe als Paarung von $\symsub{t}{i}, \symsub{delta}{i}$ für $\sym{t_vec}$ definiert, wobei $\symsub{t}{i}$ der beobachteten Zeit und $\symsub{delta}{i}$ einem Statusindikator ($\symsub{delta}{i}=1$ für einen exakten Ausfall; $\symsub{delta}{i}=0$ für eine Rechts-Zensierung) entspricht \cite{Bertsche.2022, Meeker.2022,Wu.2021,Kalbfleisch.2002,Rigdon.2022}.
...
Dies erfolgt über die \textbf{Fisher-Informationsmatrix} $\sym{FIM}$, welche die Information der Stichprobe über die Parameter $\sym{theta}$ gemäß Rechenaufwand und Qualität vergleichsweise effizient quantifiziert \cite{Kremer.2019c} und in gängigen Applikationen somit dem Standard entspricht \cite{Nelson.2005,Nelson.1990,Yang.2007}.
In der praktischen Anwendung wird die Fisher-Informationsmatrix auf Basis der resultierenden Schätzwerte $\sym{theta}=\hat{\sym{theta}}$ so als Schätzung zur Beobachtung nach $\symsub{FIM}{O}$ verwendet \cite{Nelson.1990,Lawless.2003}.
...
Diese Kovarianzen sind von entscheidender Bedeutung, da sie die statistische Abhängigkeit zwischen den Schätzwerten (z.B. zwischen $\hat{\sym{T}}$ und $\hat{\sym{b}}$) quantifizieren, welche für die Berechnung der \acp{CI} für abgeleitete Funktionen wie $\hat{\sym{R}}(\sym{t})$ erforderlich sind \cite{Meeker.2022}.
...
Dieses auf einer Taylor-Reihenentwicklung basierende Verfahren (Gauß'sche Fehlerfortpflanzung) approximiert die Varianz der Funktion $\hat{\sym{g_func}}$ unter Einbeziehung der gesamten Varianz-Kovarianz-Matrix.
...
Multivariate Lebensdauertests erfordern definitionsgemäß die Betrachtung mehrere $\sym{k}\geq 2$ Einflussfaktoren als Versuchsparameter zur Erprobung gegenüber einer Zielgröße, also der Lebensdauer.
Dementsprechend entscheidend ist das Verständnis der wesentlichen Grundlagen im Umgang mit statistischer Versuchsplanung (\ac{DoE}) für Lebensdauerdaten sowie der Lebensdauer-Modellbildung.
Da der bloße Aufbau eines Versuchsplans zudem genau die Qualität und Umfänge der Modellierung der Zielgröße in Teilen bedingt - jedenfalls aber beeinflusst, ist neben den fundamentalen Zusammenhängen auch die Bewertung von Merkmalen der Versuchspläne durch spezifische Performance-Indikatoren oder Optimalitäten relevant.
Daher beschäftigt sich der Inhalt dieses Kapitels mit einer gezielten Auswahl an Grundbegriffen und Metriken für multivariate Testpläne sowie einer Übersicht der für Lebensdauertests geeigneten Versuchspläne, bevor abschließend die statistische Modellbildung beleuchtet wird.
Für eine grundsätzlichere Auseinandersetzung zu vorgeschalteten Methoden und Werkzeugen bezüglich \ac{DoE} sei mit Blick auf den Fokus und Umfang der vorliegenden Arbeit damit auf einschlägige Literatur wie \cite{Kleppmann.2016,Siebertz.2017,Montgomery.2020,Hinkelmann.2012} verwiesen, die durch weitere am \ac{IMA} entstandenen Arbeiten reichhaltig ergänzt wird \colorbox{yellow}{Kremer, Herzig, Dazer} - um nur eine Auswahl der relevante Referenzen zu nennen. 
Daher wird im Umkehrschluss der Umgang mit normalverteilten Daten im Rahmen des \ac{DoE}, mit der Regressionsmodellierung auf Basis der \ac{ANOVA} sowie konventionellen Hypothesentests im Rahmen der nachfolgenden Ausführungen in Bezug auf Lebensdauerdaten nicht behandelt.
...
\end{itemize}
\cite{RisbergEllekjr.1998, Bisgaard.1992,Bisgaard.2011,Myers.2016,Montgomery.2020,Zahran.2003,GiovannittiJensen.1989,Goos.2011,Hinkelmann.2012,Khuri.2006,Rittmaier.2025,Johnson.2011,Donev.2004,G.E.P.Box.1951,Ardakani.2011,Jones.2012,Jones.2021,Wu.2021,Khuri.2006,Escobar.1995,Modarres.2017,Ahn.2015,Rasch.2018,Xu.2002,Wald.1943,Rencher.2008,Box.2007,Kleppmann.2016,Siebertz.2017,Fisher.1935,Bisgaard.1997,Myers.2010}