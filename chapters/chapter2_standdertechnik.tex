...
Im ingenieurwissenschaftlichen Kontext - maßgeblich gestützt durch Ausführungen in \textcite{Meeker.2022,Wu.2021,Yang.2007,Dobson.2018,Kremer.2020} - finden hingegen \textbf{Log-Location-Scale-Modelle}, also \acp{GLM}, überwiegend Anwendung.
...
\end{equation}
Sollen über die rein empirische Datenauswertung hinaus bekannte physikalische Gesetzmäßigkeiten (z.,B. Arrhenius-Gesetz oder Wöhler-Linien) im Modell berücksichtigt werden, ist die lineare Struktur des Prädiktors entsprechend anzupassen.
...
Gängige Transformationen umfassen hierbei die reziproke Anpassung für thermische Lasten oder logarithmische Ansätze für elektrische Potenzmodelle.
\begin{table}[htbp]
    \centering
    ...
    \end{tabular}
\end{table}
Eine Schätzung der unbekannten Parameter $\hat{\sym{Beta}}$ und $\hat{\sym{b}}$ erfolgt anschließend mittels \ac{MLE} durch Maximierung der Log-Likelihood-Funktion (Gleichung~\ref{eq:log_likelihood_explicit_censored}) mittels beispielsweise \textbf{Newton-Raphson-Verfahren} bzw. \textbf{Pattern-Search-Algorithmen} oder \textbf{Gradientenverfahren}, \cite{Yang.2007,Qiao.1994}.
...
Die Teststatistik $\sym{LR}$ quantifiziert den Informationsverlust durch die Modellreduktion und ist definiert als \textbf{Devianz}-Funktion \cite{Meeker.2022}:
...
Analytisch erfolgt dies über die Betrachtung der Krümmung der Log-Likelihood-Funktion $\sym{Lambda}$ im Maximum. Je stärker die Krümmung (steilerer Peak), desto präziser ist die Schätzung (geringere Varianz).
Mathematisch wird dies analog zu Gleichungen~\ref{eq:fim_o}-\ref{eq:var_covar} durch die Hesse-Matrix $\sym{H}$ beschrieben, welche die zweiten partiellen Ableitungen der Log-Likelihood-Funktion nach den Parametern $\symsub{thet}{idx_i}$ (also $\symsub{beta}{idx_j}$ und $\sym{b}$) enthält.
...
Zur analytischen Ermittlung dieser sogenannten Fisher-Vertrauensbereiche führt \textcite{Yang.2007} zunächst eine Hilfsgröße $\sym{w_aux}$ ein.
Diese normiert die logarithmische Abweichung der betrachteten Zeit $\sym{t}$ von der geschätzten charakteristischen Lebensdauer $\hat{\sym{T}}$ mit dem Formparameter $\hat{\sym{b}}$ (vergleiche \textcite{Bain.2017b} für weitere Abschätzungen):
\begin{equation} \label{eq:conf_aux_w}
    \sym{w_aux} = \hat{\sym{b}} \cdot \ln \left( \frac{\sym{t}}{\hat{\sym{T}}} \right).
\end{equation}
Die Varianz dieser Größe lässt sich mittels Taylor-Reihenentwicklung (Delta-Methode, vgl. Abschnitt~\ref{subsec:schätzer}) approximativ aus den Varianzen und Kovarianzen der Parameterschätzer bestimmen:
\begin{equation} \label{eq:var_w}
    \hat{\sym{Var}}(\sym{w_aux}) = \left( \frac{\hat{\sym{b}}}{\hat{\sym{T}}} \right)^2 \hat{\sym{Var}}(\hat{\sym{T}}) + \left( \frac{\sym{w_aux}}{\hat{\sym{b}}} \right)^2 \hat{\sym{Var}}(\hat{\sym{b}}) - \frac{2 \sym{w_aux}}{\hat{\sym{T}}} \hat{\sym{Cov}}(\hat{\sym{T}}, \hat{\sym{b}}).
\end{equation}
Basierend auf der asymptotischen Normalverteilung ergeben sich die Vertrauensgrenzen $[\sym{w_aux}_{\sym{idx_u}}, \sym{w_aux}_{\sym{idx_o}}]$ gemäß:
\begin{equation} \label{eq:conf_bounds_w}
    [\sym{w_aux}_{\sym{idx_u}}, \sym{w_aux}_{\sym{idx_o}}] = \sym{w_aux} \pm \sym{z}_{1-\sym{alpha}/2} \cdot \sqrt{\hat{\sym{Var}}(\sym{w_aux})}.
\end{equation}
Die Rücktransformation dieser Grenzen in den Wertebereich der Ausfallwahrscheinlichkeit erfolgt anschließend über die Verteilungsfunktion der kleinsten Extremwerte $\sym{G_func} = 1 - \exp[-\exp(\sym{w_aux})]$, woraus sich direkt die Konfidenzintervalle der Zuverlässigkeit ableiten lassen.
...
Die für Gleichung~\ref{eq:var_w} benötigte Varianz $\hat{\sym{Var}}(\hat{\sym{T}})$ muss folglich unter Berücksichtigung der gesamten Kovarianzmatrix der Regressionskoeffizienten berechnet werden.
...
\subsubsection{Modellgüte und Residuenanalyse}
Die Beurteilung der Modellgüte bei Lebensdauerdaten erfordert, bedingt durch Nicht-Normalverteilung und Zensierung, eine Anpassung klassischer Residualkonzepte.
Während in der linearen Regression das Bestimmtheitsmaß $R^2$ dominiert, basiert die Validierung von \acp{GLM} primär auf der Analyse der Abweichung zwischen beobachteter und modellierter kumulativer Hazard-Rate.
Hierfür haben sich insbesondere die \textbf{Cox-Snell-Residuen }$\sym{resid_CS}$ etabliert \cite{Meeker.2022}.
Sie transformieren die beobachteten Lebensdauern $\symsub{t}{idx_i}$ unter Verwendung der geschätzten Parameter $\hat{\sym{theta}}$ in einen standardisierten Raum:
\begin{equation} \label{eq:resid_cox_snell}
    {\sym{resid_CS}}_{\sym{idx_i}} = - \ln \left( \hat{\sym{R}}(\symsub{t}{idx_i}, \hat{\sym{theta}}) \right) = \left( \frac{\symsub{t}{idx_i}}{\hat{\sym{T}}_{\sym{idx_i}}} \right)^{\hat{\sym{b}}}.
\end{equation}
...
Diese Eigenschaft erlaubt eine grafische Validierung mittels Probability-Plots: Liegen die geplotteten Residuen auf einer Geraden im Exponential-Wahrscheinlichkeitsnetz, kann von einer adäquaten Modellpassung ausgegangen werden \cite{Nelson.1990}.
Zur Identifikation von Ausreißern oder einflussreichen Beobachtungen eignen sich hingegen standardisierte Devianz-Residuen $\sym{resid_Dev}$, da diese symmetrischer um Null verteilt sind und asymptotisch einer Standardnormalverteilung folgen \cite{Meeker.2022}:
...
\end{equation}
Eine Betrachtung dieser Residuen über den Index $\sym{idx_i}$ oder gegen die prädizierten Werte erlaubt das Erkennen systematischer Verzerrungen, welche auf nicht berücksichtigte Kovariaten oder eine fehlerhafte Link-Funktion hindeuten.
