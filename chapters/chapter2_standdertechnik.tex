...
Wird maximale Flexibilität jenseits starrer Standard-Designs wie dem voll-faktoriellen Setup oder dem \ac{CCD} verlangt, können optimale Versuchsplanungen als Ansatz genutzt werden.
Diese folgen dem Ziel, die Anordnung der Versuchspunkte im Versuchsraum so zu gestalten, dass spezifische Qualitätskriterien, allen voran Varianzverteilungen innerhalb der Modellierung, optimiert werden.
Eine Übersicht gängiger Metriken bzw. Qualitätsmerkmale wird daher im Folgenden gegeben.
...
\begin{equation}
    ...
    \label{eq:skalar}
\end{equation}
Oder in anderen Worten: Wenn die \textbf{Informationsmatrix}
\begin{equation}
    \sym{M}\stackrel{\sym{m}=1}{=} \sym{X}'\sym{X}
    \label{eq:informationsmatrix}
\end{equation}
einer Diagonalmatrix entspricht, ist der Versuchsplan orthogonal \cite{Montgomery.2021,Rigdon.2022}.
...
Hiermit lässt sich prüfen, ob einzelne Versuche die Schätzung eines Haupteffekts oder einer Wechselwirkung verzerren (kritischer Schwellenwert oft $> 2/\sqrt{\sym{n}}$) \cite{Belsley.2004,Montgomery.2021}.
...
Da in restriktiven Versuchsräumen Orthogonalität oft nicht erreichbar ist, zielen optimale Versuchspläne darauf ab, durch Minimierung spezifischer Eigenschaften der Dispersionsmatrix den Informationsgehalt trotz Korrelationen zu maximieren.
...
Eine Übersicht dieser Optimalitätskriterien, welche den quantitativen Vergleich zur Standard-Methodik ermöglichen, ist in Tabelle~\ref{tab:2.1_optimalitaeten} zusammengefasst und folgt in Abschnitt~\ref{subsec:stat} der zugehörigen Berechnung \cite{Goos.2011,Montgomery.2020}.

\begin{table}[htbp]
    \centering
    \caption{Übersicht und Zielsetzung verschiedener Optimalitätskriterien für Versuchspläne \cite{Goos.2011,Montgomery.2020,Myers.2016,Rigdon.2022}}
    \label{tab:2.1_optimalitaeten}
    \renewcommand{\arraystretch}{1.2} % Etwas mehr Abstand zwischen den Zeilen

    \begin{tabular}{@{}lp{0.75\textwidth}@{}}
        \toprule
        \textbf{Kriterium}     & \textbf{Zielgröße und Beschreibung}    \\
        \midrule

        \textbf{A-Optimalität} &
        Minimierung der Spur der inversen Informationsmatrix. \newline
        Ziel: Minimierung der durchschnittlichen Varianz der Regressionskoeffizienten. \newline
        \textit{Präzision der einzelnen Parameter}                      \\
        \addlinespace

        \textbf{D-Optimalität} &
        Minimierung des Volumens des gemeinsamen Vertrauensbereichs der Regressionskoeffizienten. \newline
        \textit{Beste Gesamtschätzung aller Parameter}                  \\
        \addlinespace

        \textbf{E-Optimalität} &
        Minimierung der maximalen Breite des Vertrauensbereichs eines einzelnen Regressionskoeffizienten. \newline
        \textit{Worst-Case-Absicherung für den ungenausten Parameter}   \\
        \addlinespace

        \textbf{G-Optimalität} &
        Minimierung der erwarteten, maximalen Prädiktionsvarianz im gesamten Parameterraum. \newline
        \textit{Qualität der Vorhersage an der schlechtesten Stelle}    \\
        \addlinespace

        \textbf{I-Optimalität} &
        Minimierung der durchschnittlichen Prädiktionsvarianz über den gesamten Parameterraum. \newline
        \textit{Optimale Vorhersage-Güte im Mittel; ideal für \ac{RSM}} \\
        \addlinespace

        \textbf{S-Optimalität} &
        Harmonisierung des Abstandes der Versuchspunkte im Parameterraum. \newline
        \textit{Gleichmäßige Raumfüllung / Space-Filling Design}        \\
        \addlinespace

        \textbf{V-Optimalität} &
        Minimierung der durchschnittlichen Prädiktionsvarianz über ein definiertes Set von Punkten (z.\,B. Validierungspunkte). \newline
        \textit{Optimale Vorhersage-Güte an spezifischen Stellen}       \\
