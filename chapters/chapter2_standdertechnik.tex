...
\subsubsection{Heuristisches Parameter-Screening} \label{subsubsec:screening}
...
Daraus folgend dienen Screening-Methoden und -Versuchspläne dem Ziel, Informationsverluste bei einer minimalen Anzahl an Versuchsläufen zu begrenzen und die vitalen von den trivialen Faktoren zu separieren.
...
Dies impliziert den Einsatz \textbf{heuristischer Screening-Methoden}, wie sie exemplarisch durch \textcite{Kremer.2021} und \textcite{Gundlach.2004} zusammengefasst werden.
Solche heuristischen Ansätze fundieren auf einer \textbf{Systemanalyse}, die sowohl mit als auch ohne spezifisches A-priori-Wissen über das System erfolgen kann.
...
Prozessual verorten sich heuristische Screening-Schritte zwischen der Definition des Untersuchungsziels und der Durchführung der physischen (Screening-)Experimente (vgl. \textit{Schritt~3} in \colorbox{yellow}{Abbildung~Doe-Steps}).
Mit deren Hilfe kann jedenfalls eine Priorisierung der Beeinträchtigungen der Response durch schiere Anzahl von rational begründbaren Einflüssen oder Wechselwirkungen auf die entsprechende Performance-Kennzahl, die Systemantwort, getroffen und somit eine Auswahl für fortführende Prozessschritte erfasst werden.
Als rationale Methode zur Plausibilisierung und Entscheidungsfindung können Heuristiken somit die experimentelle Charakterisierung komplexer Systemzusammenhänge teilweise substituieren - oder den damit verbundenen Aufwand zumindest  reduzieren.
