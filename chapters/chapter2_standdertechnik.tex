...
Zur lokalen Diagnose von Versuchspunkten und deren Auswirkung auf die Regressionsparameter existieren in der Fachliteratur etablierte Maßzahlen wie die Hebelwerte ($\sym{h}_{\sym{idx_i}\sym{idx_i}}$) der Prädiktionsmatrix, die Cook-Distanz ($\symsub{D}{idx_i}$) sowie detaillierte Einflussstatistiken wie $\sym{dfbetas}$, $\sym{dffits}$ und $\sym{covratio}$ - vergleiche Werke von \textcite{Siebertz.2026, Montgomery.2021, Belsley.2004, Fahrmeir.2009}.
