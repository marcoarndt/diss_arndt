...
\begin{equation} \label{eq:mle_likelihood_simple}
    \sym{L_like}(\sym{t_vec} | \sym{theta}) = \prod_{\sym{idx_i}=1}^{\sym{n}} \sym{f}(\symsub{t}{idx_i} | \sym{theta}).
\end{equation}
...
\begin{equation} \label{eq:mle_loglikelihood_simple}
    \sym{Lambda} \defeq \ln\left( \sym{L_like}(\sym{theta}) \right) = \sum_{\sym{idx_i}=1}^{\sym{n}} \ln \left[ \sym{f}(\symsub{t}{idx_i} | \sym{theta}) \right].
\end{equation}
...
\begin{equation} \label{eq:mle_likelihood_censored}
    \sym{L_like}(\sym{t_vec} | \sym{theta}) = \prod_{\sym{idx_i}=1}^{\sym{n}} \left[ \sym{f}(\symsub{t}{idx_i} | \sym{theta})^{\symsub{delta}{idx_i}} \cdot \sym{R}(\symsub{t}{idx_i} | \sym{theta})^{1 - \symsub{delta}{idx_i}} \right]
\end{equation}
und definiert die Log-Likelihood Funktion als:
\begin{equation} \label{eq:mle_loglikelihood_censored}
    \sym{Lambda} \defeq \ln\left(\sym{L_like}(\sym{t_vec} | \sym{theta})\right) = \sum_{\sym{idx_i}=1}^{\sym{n}} \left[ \symsub{delta}{idx_i} \cdot \ln \sym{f}(\symsub{t}{idx_i} | \sym{theta}) + (1 - \symsub{delta}{idx_i}) \cdot \ln \sym{R}(\symsub{t}{idx_i} | \sym{theta}) \right].
\end{equation}
...
Die planerische Kombination verschiedener Faktorstufen äußert sich in spezifischen \textbf{Versuchspunkten} innerhalb des Parameterraums und entspricht der Versuchsplan-Matrix \cite{Kleppmann.2016,Siebertz.2017}.
...
Mittels \textbf{Kontrastmethode} wird also die Änderung der Systemantwort über alle durchgeführten Versuche mit jeweiligen Faktorsrufen registriert:
...
Stoßen Standard-Designs (CCD, Box-Behnken) aufgrund von Restriktionen im Versuchsraum (\textbf{Constraints}), nicht-standardmäßigen Modellierungszielen oder ungewöhnlichen Stichprobenumfängen an ihre Grenzen, empfiehlt sich der Einsatz von \textbf{Optimalen Versuchsplänen}.
...
Optimale Versuchspläne erweitern das methodische Spektrum, indem sie eine maximale Flexibilität jenseits starrer Standard-Designs wie dem voll-faktoriellen Setup oder dem \ac{CCD} ermöglichen.
Ihr Einsatz ist prädestiniert für Szenarien, in denen klassische Pläne an ihre Grenzen stoßen - sei es durch physikalische Randbedingungen welche bestimmte Faktorstufenkombinationen ausschließen, oder durch strikte Limitierungen der verfügbaren Versuchskapazität.
...
Ein Versuchsplan wird als orthogonal bezeichnet, wenn keine Korrelation zwischen jeweils zwei Spalten der Versuchsplan-Matrix vorliegt - deren Skalarprodukt also null ergibt:
\begin{equation}
    \langle \sym{x}, \sym{x} \rangle = 0 \quad \text{für alle möglichen } \symsub{x}{idx_i}.
    \label{eq:skalar}
\end{equation}
Oder in anderen Worten: Wenn die \textbf{Informationsmatrix}
\begin{equation}
    \sym{M}\stackrel{\sym{r}=1}{=} \sym{X}^{\top}\sym{X}.
...
Dies stellt einen Versuchsplan also zunächst einmal qualitativ günstig.\
...
Mathematisch fundiert die Bewertung der Schätzgenauigkeit auf der Inversen der Informationsmatrix, welche als \textbf{Varianz-Kovarianz-Matrix} (oder \textbf{Dispersionsmatrix}) der Regressionskoeffizienten definiert ist:
\begin{equation}
    \sym{Var}(\hat{\sym{Beta}}) = \sym{sigma_sq} (\sym{X}^{\top}\sym{X})^{-1},
    \label{eq:dispersion}
\end{equation}
wobei wie gehabt $\sym{sigma_sq}$ die Varianz des Versuchsfehlers darstellt.
Die Diagonalelemente der Matrix $(\sym{X}^{\top}\sym{X})^{-1}$ stehen hierbei in direktem Zusammenhang mit dem \ac{VIF}.
Der \ac{VIF} dient als Maßzahl für Multikollinearität und quantifiziert den Faktor, um den sich die Varianz eines geschätzten Koeffizienten im Vergleich zu einem vollständig orthogonalen Design aufgrund von Korrelationen zwischen den Faktoren erhöht \cite{Siebertz.2017,Kleppmann.2016}.
Während bei orthogonalen Plänen (Idealfall) ein $\text{\ac{VIF}} = 1$ vorliegt, deuten hohe Werte (typischerweise $>5$ oder $>10$) auf eine instabile Modellschätzung hin \cite{Myers.2016,Montgomery.2021}.
Ergänzend zur globalen Bewertung der Multikollinearität durch den \ac{VIF} erlaubt die Betrachtung der sogenannten \textbf{Hat-Matrix}
\begin{equation}
    \sym{Hat} = \sym{X}(\sym{X}^{\top}\sym{X})^{-1}\sym{X}^{\top}
    \label{eq:hat_matrix}
\end{equation}
eine lokale Diagnose der Versuchspunkte.
Die Diagonalelemente $\text{h}_{ii}$ dieser Projektionsmatrix, bezeichnet als \textbf{Hebelwerte} (engl. Leverage), quantifizieren den Einfluss eines einzelnen Versuchslaufs auf die Modellvorhersage.
Punkte mit hohen Hebelwerten (typischerweise $\text{h}_{ii} > 2\sym{p}/\sym{n}$) befinden sich geometrisch weit vom Zentrum des Versuchsraums entfernt und dominieren die Regression, was den Plan anfällig für Ausreißer in diesen spezifischen Einstellungen macht \cite{Montgomery.2021, Myers.2016}.
...
Insbesondere bei reduzierten Versuchsplänen ist die Power-Analyse essenziell, um das Risiko von $\sym{Beta}$-Fehlern (falsch-negativen Ergebnissen) zu minimieren und sicherzustellen, dass die gewählte Stichprobengröße $\sym{n}$ für den angestrebten Erkenntnisgewinn hinreichend ist \cite{Cohen.1988, Siebertz.2017}.\
...
Eine Übersicht dieser Optimalitätskriterien, welche den quantitativen Vergleich zur Standard-Methodik ermöglichen, ist in Tabelle~\ref{tab:2.1_optimalitäten} zusammengefasst und folgt in Abschnitt~\ref{subsec:stat} der zugehörigen Berechnung \cite{Goos.2011,Montgomery.2020}.
\begin{table}[htbp]
    \centering
    ...
    \begin{tabular}{@{}lp{0.75\textwidth}@{}}
        ...
        \textbf{A-Optimalität} &
        Eigenschaft: Spur der invertierten \newline
        Minimierung der durchschnittlichen Breite des Vertrauensbereichs der Regressionskoeffizienten. \newline
        ...
        \textbf{V-Optimalität} &
        Minimierung der Prädiktionsvarianz über ein Set von Punkten, z.B. die Versuchspunkt selbst, im Parameterraum. \newline
        \textit{Optimale Vorhersage-Güte in Punkteset}                  \\
        \addlinespace
        \bottomrule
    \end{tabular}
    \label{tab:2.1_optimalitäten}
\end{table}
...
In der Praxis entsteht ein \ac{CCD} häufig im Rahmen einer \textbf{sequenziellen Versuchsstrategie}: Zeigt das initiale lineare Modell („First-Order Model“) Anpassungsmängel (\textbf{Lack-of-Fit}), wird der bestehende faktorielle Plan um Sternpunkte erweitert („augmentiert“), um ein Modell zweiter Ordnung zu fitten \cite{Myers.2016}.
Prinzipielle Vorgehensweisen, die ein erfolgreiches umsetzen von \ac{DoE} begünstigen sollten, wie \textbf{Blockbildung} oder \textbf{Randomisierung} (vgl. \cite{Siebertz.2017,Kleppmann.2016}) seien hier vorausgesetzt.
