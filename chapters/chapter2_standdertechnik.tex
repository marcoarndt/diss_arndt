...
Stoßen Standard-Designs (CCD, Box-Behnken) aufgrund von Restriktionen im Versuchsraum (\textbf{Constraints}), nicht-standardmäßigen Modellierungszielen oder ungewöhnlichen Stichprobenumfängen an ihre Grenzen, empfiehlt sich der Einsatz von \textbf{Optimalen Versuchsplänen}, vergleiche übernächsten Abschnitt.
...
Modelle zweiter Ordnung sind insbesondere in der Lebensdauer- und Zuverlässigkeitstechnik vermehrt von Bedeutung, da sie die Existenz von lokalen Extrema (Minima oder Maxima) der Systemantwort im Versuchsraum abbilden können \cite{Rigdon.2022}.
...
Abbildung~\ref{fig:ma_abb2.08_strategy} fasst eine derartige strategische Vorgehensweise zur Realisierung eines \ac{CCD} zusammen, wie sie \textcite{Box.1988} vorschlagen.
...
\end{figure}
Demnach wird zunächst ein \textbf{teil-faktorieller Versuchsplan} zur Identifikation signifikanter Faktoren durchgeführt.
...
Eine Übersicht der einschlägigsten \textbf{Optimalitätskriterien} - oder einfach \textbf{Optimalitäten} - welche den quantitativen Vergleich zur Standard-Methodik ermöglichen, ist in Tabelle~\ref{tab:2.1_optimalitaeten} zusammengefasst \cite{Goos.2011,Montgomery.2020}. Sie bilden mitunter die Grundlage für diverse weitere Optimalitäten und hybride Ansätze \cite{Myers.2016,Rigdon.2022}.
{
...
\end{equation}
        ...
        \textbf{D-Optimalität} &
        Maximierung der Determinante der Informationsmatrix $\sym{M}$ \cite{Kleppmann.2020,Montgomery.2020,Rigdon.2022}. \newline
        ...
        \textbf{E-Optimalität} &
        Minimierung des maximalen Eigenwerts $\sym{eigen_val}_{\max}$ der inversen Informationsmatrix \colorbox{yellow}{\cite[Erich->Boyd]{Kleppmann.2020}}. \newline
        ...
        \textbf{G-Optimalität} &
        Minimierung der maximalen skalierten Prädiktionsvarianz $\sym{pred_var}$ im relevanten Parameterraum \cite{Montgomery.2020,Myers.2016}. \newline
        ...
        \textbf{I-Optimalität} &
        Minimierung der durchschnittlichen (\textit{integrierten}) Prädiktionsvarianz über den Parameterraum - auch \textbf{U-Optimalität} im ingenieurwissenschaftlich-statistischen Sinn: (\textit{uniforme} Prädiktionsvarianz) \colorbox{yellow}{\cite[Monroe]{Rigdon.2022}}. \newline
        ...
        \textbf{V-Optimalität} &
        Minimierung der durchschnittlichen Prädiktionsvarianz über ein diskretes Set von $\sym{N}$ Punkten - z.B. die \ac{L-DoE}-Versuchspunkte oder sepzifische Nennlasten \cite{Goos.2011,Montgomery.2020,Myers.2010}. \newline
...
Die Trennschärfe beschreibt als Metrik diejenige Wahrscheinlichkeit, mit der ein existierender Einfluss durch Faktoren vor dem Hintergrund des experimentellen Rauschens korrekt als \textbf{signifikanter} Effekt $\sym{Eff}$ identifiziert wird \cite{Siebertz.2017,Kleppmann.2020}.
...
Der Anteil der Simulationsläufe, in denen der definierte Effekt korrekt als signifikant ($\text{p-Wert} < \sym{alpha}$) erkannt wird, entspricht der geschätzten $\sym{power}$ des Designs.


\subsection{Statistische Modellbildung} \label{subsec:model}

\begin{itemize}
    \item -> todo next: \newline https://www.researchgate.net/profile/Steven-Rigdon/research \newline https://www.researchgate.net/profile/Rong-Pan-7/research \newline https://www.researchgate.net/scientific-contributions/Eric-M-Monroe-70325879 \newline \cite[S.314]{Rigdon.2022}: UC Optimal Test Deisgns
