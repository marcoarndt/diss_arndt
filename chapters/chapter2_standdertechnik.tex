...
Mittels \textbf{Kontrastmethode} wird also die Änderung der Systemantwort über alle durchgeführten Versuche mit jeweiligen Faktorsrufen registriert \cite{Kleppmann.2020,Montgomery.2020}:
...
Der erste Schritt zur Detektion von Nichtlinearitäten besteht in der Integration von $\symsub{n}{idx_C}$ sogenannten \textbf{Zentralpunkten}, engl. \acp{CP}, in den faktoriellen Basisplan. Hierbei werden alle Faktoren auf die kodierte Stufe \textit{0} (die Mitte des Versuchsraums) gesetzt.
...
\end{equation}
Alternativ kann für sphärische Räume auch $\symsub{alpha}{idx_D} = \sqrt{\sym{k}}$ gewählt werden (\textbf{Sphärisches CCD}), wodurch alle Versuchs- und Sternpunkte auf einer Kugeloberfläche liegen \cite{Myers.2016}.
Ist der Versuchsraum hingegen durch harte physikalische Grenzen (z.B. maximale Temperatur) kubisch beschränkt, bietet sich der \textbf{flächenzentrierte CCD} (\textbf{Face Centered CCD}) an.
...
Eine effiziente Alternative zum CCD stellt das \textbf{Box-Behnken-Design} dar \cite{Box.1960}.
...
Stoßen Standard-Designs (CCD, Box-Behnken) aufgrund von Restriktionen im Versuchsraum (\textbf{Constraints}), nicht-standardmäßigen Modellierungszielen oder ungewöhnlichen Stichprobenumfängen an ihre Grenzen, empfiehlt sich der Einsatz von \textbf{Optimalen Versuchsplänen}, vergleiche nachfolgenden Abschnitt.
...
Wird maximale Flexibilität jenseits starrer Standard-Designs wie dem voll-faktoriellen Setup oder dem \ac{CCD} verlangt, bieten \textbf{optimale Versuchspläne} einen leistungsfähigen Lösungsansatz.
Diese verfolgen das Ziel, die Versuchspunkte algorithmisch so im Versuchsraum zu positionieren, dass spezifische Qualitätskriterien - allen voran die Verteilung der Prädiktionsvarianz
\begin{equation}
    \sym{pred_var} = \sym{x}' (\sym{X}'\sym{X})^{-1} \sym{x} = \sym{x}' \sym{M}^{-1} \sym{x}.
    \label{eq:pred_variance_def}
\end{equation}
optimiert werden.
Die Prädiktionsvarianz ist ein dimensionsloses Maß, das ausschließlich von der geometrischen Anordnung der Versuchspunkte (der Matrix $\sym{X}$) und der Position $\sym{x}$ abhängt, jedoch unabhängig von der tatsächlichen Streuung der Messdaten ($\sym{sigma_sq}$) ist.
...
\begin{equation}
    ...
    \label{eq:skalar}
\end{equation}
Oder in anderen Worten: entpricht die \textbf{Informationsmatrix}
\begin{equation}
    \sym{M}\stackrel{\sym{m}=1}{=} \sym{X}'\sym{X}
    \label{eq:informationsmatrix}
\end{equation}
einer Diagonalmatrix, ist der Versuchsplan orthogonal \cite{Montgomery.2021,Rigdon.2022}.
...
Die Diagonalelemente der Matrix $(\sym{X}'\sym{X})^{-1}$ stehen hierbei in direktem Zusammenhang mit dem \ac{VIF}.
Der \ac{VIF} dient als Maßzahl für Multikollinearität und quantifiziert den Faktor, um den sich die Varianz eines geschätzten Koeffizienten im Vergleich zu einem vollständig orthogonalen Design aufgrund von Korrelationen zwischen den Faktoren erhöht \cite{Siebertz.2017,Kleppmann.2020}.
...
Eine Übersicht dieser \textbf{Optimalitätskriterien} - oder einfach \textbf{Optimalitäten}, welche den quantitativen Vergleich zur Standard-Methodik ermöglichen, ist in Tabelle~\ref{tab:2.1_optimalitaeten} zusammengefasst \cite{Goos.2011,Montgomery.2020}.

\begin{table}[htbp]
    \centering
    \caption{Übersicht, Zielsetzung und mathematische Definition verschiedener Optimalitätskriterien für Versuchspläne \cite{Goos.2011,Montgomery.2020,Myers.2016,Rigdon.2022}}
    \label{tab:2.1_optimalitaeten}
    \renewcommand{\arraystretch}{1.2} % Zeilenabstand

    % Lokaler Befehl für kompaktere Formeln in der Tabelle
    \newcommand{\compressEq}{\vspace*{-0.6\baselineskip}}
    ...
    \begin{tabular}{@{}lp{0.75\textwidth}@{}}
        ...
        \textbf{A-Optimalität} &
        Minimierung der Spur der inversen Informationsmatrix. \newline
        \textit{Ziel: Maximale Präzision der einzelnen Parameter im Durchschnitt.}
        \compressEq
        \begin{equation} \label{eq:opt_A}
            \sym{A_opt} = \min \left( \text{spur}(\sym{M}^{-1}) \right) = \min \left( \sum_{j=1}^{\sym{k}} \sym{Var}(\hat{\beta}_j) \right)
        \end{equation}
        \compressEq
        \\ \addlinespace

        \textbf{D-Optimalität} &
        Maximierung der Determinante der Informationsmatrix $\sym{M}$. \newline
        \textit{Ziel: Beste Gesamtschätzung; minimiert das Volumen des Vertrauensellipsoids.}
        \compressEq
        \begin{equation} \label{eq:opt_D}
            \sym{D_opt} = \max \left( \det(\sym{M}) \right) \Leftrightarrow \min \left( \det(\sym{M}^{-1}) \right)
        \end{equation}
        \compressEq
        \\ \addlinespace

        \textbf{E-Optimalität} &
        Minimierung des maximalen Eigenwerts $\lambda_{\max}$ der inversen Informationsmatrix. \newline
        \textit{Ziel: Worst-Case-Absicherung für den am schlechtesten geschätzten Parameter.}
        \compressEq
        \begin{equation} \label{eq:opt_E}
            \sym{E_opt} = \min \left( \lambda_{\max}(\sym{M}^{-1}) \right)
        \end{equation}
        \compressEq
        \\ \addlinespace

        \textbf{G-Optimalität} &
        Minimierung der maximalen skalierten Prädiktionsvarianz $\sym{pred_var}$ im relevanten Parameterraum. \newline
        \textit{Ziel: Qualität der Vorhersage an der ungünstigsten Stelle sichern.}
        \compressEq
        \begin{equation} \label{eq:opt_G}
            \sym{G_opt} = \min \left( \max_{\sym{x} \in \sym{RR}^{\sym{k}}} \sym{pred_var} \right)
        \end{equation}
        \compressEq
        \\ \addlinespace

        \textbf{I-Optimalität} &
        Minimierung des Integrals der skalierten Prädiktionsvarianz über den Parameterraum. \newline
        \textit{Ziel: Optimale Vorhersagegüte im Mittel über den gesamten Raum.}
        \compressEq
        \begin{equation} \label{eq:opt_I}
            \sym{I_opt} = \min \left( \int_{\sym{RR}^{\sym{k}}} \sym{pred_var} \, d\sym{x} \right)
        \end{equation}
        \compressEq
        \\ \addlinespace

        \textbf{S-Optimalität} &
        Maximierung des minimalen euklidischen Abstandes zwischen zwei Versuchspunkten. \newline
        \textit{Ziel: Gleichmäßige Raumfüllung (Space-Filling).}
        \compressEq
        \begin{equation} \label{eq:opt_S}
            \sym{S_opt} = \max \left( \min_{i \neq j} \| \symsub{x}{idx_i} - \symsub{x}{idx_j} \| \right)
        \end{equation}
        \compressEq
        \\ \addlinespace

        \textbf{V-Optimalität} &
        Minimierung der durchschnittlichen Prädiktionsvarianz über ein diskretes Set von $m$ Punkten. \newline
        \textit{Ziel: Optimale Vorhersagegüte an spezifischen Stellen.}
        \compressEq
        \begin{equation} \label{eq:opt_V}
            \sym{V_opt} = \min \left( \frac{1}{m} \sum_{i=1}^{m} \sym{pred_var}(\symsub{x}{idx_i}) \right)
        \end{equation}
        \compressEq
        \\
...
\todo{
    \begin{itemize}
        \item Effizienzen (D- und G-Effizienz) \cite[Seite. 523 (2017)]{Montgomery.2020}
        \item Konditionszahl \cite[9.4.3 (2012)]{Montgomery.2021}
        \item PRESS und $R^2_{pred}$
    \end{itemize}
}\
\colorbox{yellow}{--TODO--}


% 1. Die Konditionszahl (Condition Number)Während der VIF die Multikollinearität für einzelne Koeffizienten prüft, bewertet die Konditionszahl die numerische Stabilität der gesamten Informationsmatrix $X'X$.Was sie misst: Sie gibt an, wie empfindlich die Lösung des Gleichungssystems (die Invertierung) auf kleine Änderungen in den Daten reagiert (Schlecht konditionierte Matrix).Mathematik: Sie ist das Verhältnis des größten zum kleinsten Eigenwert ($\lambda_{max}/\lambda_{min}$) der Matrix $X'X$ (bzw. Singularwerte bei nicht-quadratischen Matrizen).Bewertung:Wert = 1: Perfekt orthogonal.Wert < 100: Gut.Wert > 1000: Starke Multikollinearität, das Modell ist numerisch instabil.
% 2. Design-Effizienzen (G-Effizienz, D-Effizienz)Die reinen Optimalitätswerte (z. B. Determinante $|X'X|$ für D-Optimalität) sind schwer zu interpretieren, da sie von der Anzahl der Versuche ($n$) und der Skalierung abhängen. Um Pläne unterschiedlicher Größe vergleichbar zu machen, nutzt man Effizienzen (normiert auf 0 bis 100\%).D-Effizienz: Vergleicht das Design mit einem hypothetischen, perfekt orthogonalen Design gleicher Größe.$$\text{D-Eff} = \left( \frac{|\sym{X}'\sym{X}|}{n^p} \right)^{1/p}$$G-Effizienz: Setzt die theoretische Anzahl der Parameter ins Verhältnis zur maximalen Varianz im Designraum (bezogen auf G-Optimalität).$$\text{G-Eff} = \frac{p}{n \cdot \max(\text{SPV})}$$Nutzen: Damit können Sie bewerten: "Mein Plan ist zu 95\% D-effizient, aber ich spare 5 Versuche gegenüber dem Standardplan."
% 3. PRESS und $R^2_{\text{pred}}$ (Prognosegüte)Da Sie spezifisch "Prädiktion" angesprochen haben: Die klassischen $R^2$-Werte sagen nur, wie gut das Modell die vorhandenen Daten erklärt (Fitting). Um zu prüfen, wie gut es neue Daten vorhersagt, nutzt man die PRESS-Statistik (Predicted Residual Error Sum of Squares).Konzept: Basiert ebenfalls auf Leave-One-Out. Man summiert die quadrierten Fehler der Vorhersage für jeden weggelassenen Punkt.$R^2_{\text{pred}}$: Ein daraus abgeleitetes Bestimmtheitsmaß für die Vorhersage.Wenn $R^2_{\text{pred}}$ deutlich kleiner ist als das normale $R^2$ (z. B. Differenz > 0.2), ist das Modell überangepasst (Overfitting) oder ein einzelner Punkt (siehe COVRATIO/Cook's D) verzerrt die Vorhersagekraft massiv.


Zuletzt muss sichergestellt werden, dass der Versuchsplan über eine ausreichende \textbf{statistische Power} (Teststärke) verfügt.
Diese Metrik beschreibt die Wahrscheinlichkeit, mit der ein Effekt einer definierten Größe (Signal) vor dem Hintergrund des experimentellen Rauschens (Noise) als signifikant erkannt wird.
Insbesondere bei reduzierten Versuchsplänen ist die Power-Analyse essenziell, um das Risiko von $\sym{Beta}$-Fehlern (falsch-negativen Ergebnissen) zu minimieren und sicherzustellen, dass die gewählte Stichprobengröße $\sym{n}$ für den angestrebten Erkenntnisgewinn hinreichend ist \cite{Siebertz.2017}.
...
In der Praxis entsteht ein \ac{CCD} häufig im Rahmen einer \textbf{sequenziellen Versuchsstrategie}: Zeigt das initiale lineare Modell Anpassungsmängel (\textbf{Lack-of-Fit}), wird der bestehende faktorielle Plan um Sternpunkte erweitert („augmentiert“), um ein Modell zweiter Ordnung zu fitten \cite{Myers.2016}.
Prinzipielle Vorgehensweisen, die ein erfolgreiches umsetzen von \ac{DoE} begünstigen sollten, wie \textbf{Blockbildung} oder \textbf{Randomisierung} (vgl. \cite{Siebertz.2017,Kleppmann.2020}) seien hier vorausgesetzt.
Ein \ac{CCD} besteht somit aus:
\begin{itemize}
    \item den $\symsub{n}{idx_F}$ Eckpunkten des ursprünglichen voll- (oder teil-)faktoriellen Plans,
    \item einer definierten Anzahl $\symsub{n}{idx_C}$ an wiederholten Zentralpunkten (üblicherweise $3 \leq \symsub{n}{idx_C} \leq 5$ zur Abschätzung des reinen Fehlers) sowie
    \item $\symsub{n}{idx_S} = 2 \cdot \sym{k}$ zusätzlichen \textbf{Sternpunkten} (engl. \textbf{Axial Points}), die auf den Achsen des Koordinatensystems im Abstand $\pm \symsub{alpha}{idx_D}$ vom Zentrum liegen.
\end{itemize}
\begin{itemize}
...
\begin{figure}[htbp]
    ...
    \end{minipage}
    \caption{Strategischer Ansatz zu augmentierter Versuchsplanung für die Lebensdauererprobung nach \cite{Box.1988}}
