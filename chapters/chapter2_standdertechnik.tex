...
\begin{equation} \label{eq:a2_indicator}
    \sym{A2} = \sum_{\sym{idx_i}=1}^{\sym{p}} \sum_{\substack{\sym{idx_j}=1 \\ \sym{idx_j} \neq \sym{idx_i}}}^{\sym{p}} \sym{corr}_{\sym{idx_i}\sym{idx_j}}^2 \quad \text{mit} \quad \sym{corr}_{\sym{idx_i}\sym{idx_j}} = \frac{\sym{M}_{\sym{idx_i}\sym{idx_j}}}{\sqrt{\sym{M}_{\sym{idx_i}\sym{idx_i}}\sym{M}_{\sym{idx_j}\sym{idx_j}}}},
\end{equation}
wobei $\sym{corr}_{\sym{idx_i}\sym{idx_j}}$ den paarweisen Korrelationskoeffizienten zwischen der $\sym{idx_i}$-ten und $\sym{idx_j}$-ten Spalte der Versuchsplan-Matrix repräsentiert.
