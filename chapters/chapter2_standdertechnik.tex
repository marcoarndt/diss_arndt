...
Ein universelles Maß zur Beurteilung der Schätzgüte einzelner Parameter ist der \textbf{\ac{MSE}}. Er vereint die Varianz des Schätzers und dessen systematische Verzerrung (\textbf{Bias}) in einer Metrik:
\begin{equation}
...
\end{equation}
Da klassische Schätzverfahren (wie \ac{OLS} oder \ac{MLE}) unter regulären Bedingungen erwartungstreu sind ($\text{Bias} = 0$), entspricht der \ac{MSE} in diesen Fällen exakt der Schätzvarianz $\sym{Var}(\hat{\sym{beta}}_{\sym{idx_j}})$ und skaliert folglich direkt mit den Diagonalelementen der inversen Informationsmatrix.
