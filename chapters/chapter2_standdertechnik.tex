...
Diese verfolgen das Ziel, die Versuchspunkte algorithmisch so im Versuchsraum zu positionieren, dass spezifische Qualitätskriterien - allen voran die Verteilung der (um die Versuchsanzahl $\sym{N}$ \textbf{skalierte}) \textbf{Prädiktionsvarianz}
\begin{equation}
    \sym{pred_var}= \frac{\sym{N} \sym{Var}[\hat{\sym{y}}(\symsub{x_loc}{idx_0})]}{\sym{sigma_sq}}
    = \sym{x}_0' \left( \frac{\sym{M}}{\sym{N}} \right)^{-1} \sym{x}_0
    = \sym{N} \sym{x}_0' \sym{M}^{-1} \sym{x}_0
    \label{eq:pred_variance_scaled}
\end{equation}
optimiert werden \cite{Myers.2016,Montgomery.2020}.
Die Prädiktionsvarianz ist ein dimensionsloses Maß, das ausschließlich von der geometrischen Anordnung der \sym{N} Versuchspunkte (der Matrix $\sym{X}$) und der Position $\symsub{x_loc}{idx_0}'=[1,\sym{x}_{01},\cdots,\sym{x}_{0\sym{k}}]$ als Referenz im Parameterraum abhängt, also entsprechend noch unabhängig von der tatsächlichen Streuung der Messdaten ($\sym{sigma_sq}$) ist.
...
Die Diagonalelemente der Matrix $(\sym{X}'\sym{X})^{-1}$ stehen hierbei in direktem Zusammenhang mit dem \ac{VIF} \cite{Siebertz.2017,Kleppmann.2020}.
...
Diese Metrik beschreibt die Wahrscheinlichkeit, mit der ein Effekt einer definierten Größe (Signal) vor dem Hintergrund des experimentellen Rauschens als \textbf{signifikant} erkannt wird.
