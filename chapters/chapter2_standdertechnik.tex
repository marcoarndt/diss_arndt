...
\begin{figure}[htbp]
    ...
    \caption{\ac{DoE} Prozess-Schritte gemäß \cite{Coleman.1993}}
...
Darin abgebildete Einflussfaktoren sollen also per se \replaced[id=Daz]{auf statistische Signifikanz bewertbar}{statistisch signifikant und somit relevant} für das Systemverhalten sein.
...
Im Zentrum der Betrachtung steht damit generell ein technisches \textit{System}, welches abstrakt als Produkt oder Prozess verstanden wird und den Zustand der Ausgangsgröße in Abhängigkeit der einwirkenden Eingangsgrößen definiert.
Die zu untersuchende oder zu optimierende Ausgangsgröße wird als \textit{Systemantwort} $\sym{y}$ bezeichnet.
Die gezielt kontrollierbaren und variierten Eingangsgrößen sind \textit{Faktoren} (\textit{Steuergrößen}), während nicht kontrollierbare oder unbekannte Einflüsse als \textit{Störgrößen} klassifiziert werden (\textit{Schritt~2}, vgl. \cite{Kleppmann.2020}).
Eine visuelle Aufstellung des genannten Zusammenspiels der Parameter kann dem Parameterdiagramm, kurz \textit{P-Diagramm}, in Abbildung~\ref{fig:ma_abb2.03_p_diagramm} entnommen werden \cite{Montgomery.2020}.
\begin{figure}[htbp]
    \centering
...
\end{figure}
Um das Systemverhalten zu charakterisieren, werden die Faktoren als \replaced[id=Daz]{diskete}{kategoriale} oder kontinuierliche Parameter im Versuch auf diskreten Werten, den sogenannten \textit{Faktorstufen}, variiert (\textit{Schritt~3}).
...
Die planerische Kombination verschiedener Faktorstufen äußert sich in spezifischen \textit{Versuchspunkten} innerhalb des Parameterraums und entspricht der Versuchsplan-Matrix \cite{Kleppmann.2020,Siebertz.2026}.
Der \textit{Test-Parameterraum} (auch \textit{Design Space}) wird hierbei durch die Gesamtheit der technisch realisierbaren und im Versuch einstellbaren Parameterkombinationen aufgespannt.
...
Die aus der Variation resultierende Änderung der Systemantwort quantifiziert den Einfluss des Faktors, der statistisch als \textit{Effekt} $\sym{Eff}$ bezeichnet wird und den Mittelwertunterschiede zweier Faktorstufen beschreibt (\textit{Schritte~6-7}).
Mittels \textit{Kontrastmethode} wird also die Änderung der Systemantwort über alle durchgeführten Versuche mit jeweiligen Faktorstufen registriert \cite{Kleppmann.2020,Montgomery.2020}:
\begin{equation} \label{eq:eff}
    \sym{Eff}=\frac{1}{\sym{n}|_{+1}}\sum{\sym{y}|_{+1}} - \frac{1}{\sym{n}|_{-1}}\sum{\sym{y}|_{-1}}
\end{equation}
So können mittels \ac{DoE} strukturiert, effizient und verbindlich Informationen gewonnen werden, die über die direkten Effekte hinausgehen und differenziert Aufschluss über \textit{Haupteffekte} sowie etwaige \textit{Wechselwirkungen} der Faktoren auf die Antwort des Systems geben - vergleiche Abb.~\ref{fig:ma_abb2.04_effekt} sowie \textcite{Montgomery.2020,Kleppmann.2020,Siebertz.2026,Kremer.2021}.
Abbildung~\ref{fig:ma_abb2.04_effekt} visualisiert derartige Effekte.
So gibt die Darstellung eines Haupteffekts in Abhängigkeit des Vorzeichens und der Steigung (\textit{positiver} oder \textit{negativer} Haupteffekt) sozusagen die Einflussstärke und -richtung wieder, während bei Wechselwirkung der Effekt in Abhängigkeit der Einstellung eines \textit{Co-Faktors} dargestellt wird.
\begin{figure}[h]
    \centering
...
\end{figure}
Diese Zusammenhänge können mathematisch positiv oder negativ beschrieben sowie durch Polynomfunktionen höherer Ordnung approximiert werden, um zusätzlich beispielsweise \textit{quadratische Effekte} oder \textit{Mehrfachwechselwirkungen} abzubilden.
...
Falls relevant, erfolgt die Ergänzung um die jeweilige Wechselwirkung und einen Fehlerterm $\sym{epsilon}$ als Zufallsvariable für Abweichungen durch \replaced[id=Daz]{Störgrößen}{Mess- und Streufehler} \cite{Montgomery.2020,Myers.2016,Rigdon.2022}.
Für $\sym{k}=2$ Faktoren resultiert daraus:
...
Sollen zunächst perspektivisch relevante Faktoren für eine versuchstechnische Untersuchung identifiziert werden, kann ein \textit{Parameter-Screening} durchgeführt werden.
...
Angesichts der potenziell hohen Komplexität durch Wechselwirkungen und Nichtlinearitäten sind die in Abbildung~\ref{fig:ma_abb2.02_doe_steps} beschriebenen \textit{Schritte~2-3} als \replaced[id=Daz]{vorbereitende}{propädeutische} Arbeiten für ein effizientes Testdesign zu interpretieren.
Methodisch lassen sich diese unter dem Terminus \textit{Screening} subsumieren.
...
Daraus folgend dienen Screening-Methoden und -Versuchspläne dem Ziel, Informationsverluste bei einer minimalen Anzahl an Versuchsläufen zu begrenzen und \textit{Steuergrößen} von den \textit{Störgrößen} zu separieren, vgl. Abbildung~\ref{fig:ma_abb2.03_p_diagramm}.
...
Standardprotokolle aus dem \ac{DoE} wie der \textit{$2^{\sym{k}}$ voll-faktorielle Versuchsplan} eignen sich grundsätzlich auch für Lebensdaueruntersuchungen, da sich hier analog zu vergleichbar statistisch verteilter Datenlage Effekte stets als (Mittelwert-) Unterschiede in der Beobachtung der Systemantwort aus dem Vergleich zweier Einstellstufen eines oder mehrerer Faktoren ergeben \cite{Kleppmann.2020}.
...
\begin{figure}[h]
    ...
    \end{subfigure}
    \caption{Standard voll-faktorieller Versuchsplan für \highlight[id=Mell]{$\sym{k}=2$} Faktoren}
...
Der voll-faktorielle Versuchsplan bildet somit den Standard-Versuchsplan im \ac{DoE} und fordert bei einmaliger Durchführung (\textit{Replikation} $\sym{m}=1$)
\begin{equation}
    \sym{n}=2^{\sym{k}}
    \label{eq:ffvp_n}
\end{equation}
Versuche.
Dieser Stichprobenumfang stellt sicher, dass das resultierende Gleichungssystem \textit{gesättigt} ist: Mit $\sym{n}$ Versuchen lassen sich $\sym{n}-1$ Effekte für Hauptfaktoren und Wechselwirkungen \textit{eindeutig} bestimmen.
...
Die Differenz der gewählten Level muss bereits im Vorfeld definiert werden, sodass signifikante Effekte sicher detektiert werden - vgl. \textit{\ac{SNR}} \cite{Montgomery.2021} - wobei gleichzeitig zu geringe Abstände (Rauschen) sowie zu große Intervalle (Gefahr unerkannter Nichtlinearitäten) zu vermeiden sind, siehe auch Abbildung~\ref{fig:ma_abb2.06_effektabstand} sowie \cite{Wu.2021,Siebertz.2026,Kleppmann.2020}.
\begin{figure}[htbp]
    \centering
...
\subsubsection{$2^{\sym{k}-\sym{p_f}}$ Teil-faktorielle Versuchspläne}
...
Für derartige Selektionsaufgaben eignen sich daher \textit{Screening-Versuchspläne}, wie der \textit{teil-faktorielle Versuchsplan} (Fractional Factorial Design) oder alternativ der \textit{Plackett-Burman-Plan} \cite{Kleppmann.2020,Siebertz.2026,Montgomery.2020}.
...
So sind beispielsweise Haupteffekte unter Umständen nicht mehr zweifelsfrei von Wechselwirkungen höherer Ordnung zu unterscheiden. Da sich diese Effekte statistisch überlagern, spricht man von einer \textit{Vermengung} (engl. \textit{Aliasing}).
Die Schwere dieser Vermengung wird dabei über die \textit{Auflösung} (engl. Resolution) des Versuchsplans klassifiziert (z.\,B. Auflösung III, IV oder V).
...
Im Sinne einer ressourceneffizienten Gesamtstrategie sollten die Screening-Versuche daher idealerweise so konzipiert sein, dass sie nahtlos in einen nachfolgenden, höher aufgelösten Versuchsplan integriert (\textit{augmentiert}) werden können.

\subsubsection{Wirkungsflächenversuchspläne} \label{subsubsec:rsm}
Die bisher diskutierten Versuchspläne beschränken sich auf die Untersuchung von Faktoren auf jeweils zwei Stufen ($\pm 1$). Dies ermöglicht zwar eine effiziente Darstellung linearer Beziehungen und Interaktionen, jedoch ist die Modellierung komplexerer, nicht-linearer Effekte aufgrund fehlender Stützstellen im Versuchsraum damit \replaced[id=Daz]{mathematisch}{physikalisch} nicht möglich.
Perspektivisch ist daher entscheidend, wie die bestehende Datenbasis weitergenutzt und augmentiert werden kann, falls die Systemantwort signifikante \textit{Krümmungen} aufweist und die quantitativen Beziehungen zwischen Faktoren und Zielgröße für eine Optimierung detaillierter beschrieben werden müssen.
...
Ein \ac{CCD} entsteht durch die \textit{Augmentierung} des ursprünglichen voll- (oder teil-)faktoriellen Plans (den $\symsub{n}{idx_F}$ Eckpunkten), vgl. Abbildung~\ref{fig:abb2.07_ccd_design}, um:
\begin{itemize}
    \item eine definierte Anzahl $\symsub{n}{idx_C}$ an wiederholten \acp{ZP} (üblicherweise $3 \leq \symsub{n}{idx_C} \leq 5$ zur Abschätzung des reinen Fehlers) sowie
...
\end{equation}
Alternativ kann für sphärische Räume auch $\symsub{alpha}{idx_D} = \sqrt{\sym{k}}$ gewählt werden (\textit{Sphärisches \ac{CCD}}), wodurch alle Versuchs- und \acp{SP} auf einer Kugeloberfläche liegen \cite{Myers.2016}.
...
Stoßen Standard-Designs (\ac{CCD}, Box-Behnken) aufgrund von Restriktionen im Versuchsraum, nicht-standardmäßigen Modellierungszielen oder ungewöhnlichen Stichprobenumfängen an ihre Grenzen, empfiehlt sich der Einsatz von \textit{Optimalen Versuchsplänen}, vergleiche übernächsten Abschnitt.
...
In der Praxis entsteht ein \ac{CCD} häufig im Rahmen einer \textbf{sequenziellen Versuchsstrategie}: Zeigt das initiale lineare Modell Anpassungsmängel (\textbf{Lack-of-Fit}), wird der bestehende faktorielle Plan um die \acp{SP} augmentiert, um ein Modell zweiter Ordnung zu fitten \cite{Myers.2016}.
...
Prinzipielle Vorgehensweisen, die ein erfolgreiches Umsetzen von \ac{DoE} bzw. \ac{L-DoE} begünstigen sollten, wie \textbf{Blockbildung} oder \textbf{Randomisierung} (vgl. \cite{Siebertz.2026,Kleppmann.2020}), seien hier bereits vorausgesetzt.
...
Daraufhin werden \textbf{\acp{ZP}} ergänzt, um das Vorhandensein von Krümmungen zu validieren.
Abschließend werden bei Bedarf die \textbf{\acp{SP}} hinzugefügt, um ein vollständiges \ac{CCD} zu realisieren.
...
Hierbei kann sequenziell ausgewertet und entschieden werden, inwiefern ein reduziertes Modell mit ausgewählten Termen gegenüber einem vollen Modell quadratischer Ordnung überlegen ist, um die Systemantwort präzise abzuschätzen \cite{Paschal.2024}.
...
In Abhängigkeit der Anpassung zur Wahl eines geeigneten Modells, linear oder quadratisch, kann die Versuchsplanung so stets an die aktuellen Erkenntnisse angepasst werden.
Dies folgt im Wesentlichen einer übergeordneten Logik der Modellkomplexität:
Gemäß \textcite{Russell.2019} korrespondiert die minimale Anzahl notwendiger Versuchspunkte direkt mit der Anzahl der zu schätzenden Modellparameter $\sym{p}$.
Intuitiv erfordert bereits die Bestimmung der Parameter einer einfachen Geraden ($\symsub{beta}{idx_0}, \sym{beta}_1$) mindestens $\sym{N}=2$ unterschiedliche Stützstellen.
Soll hingegen ein quadratischer Zusammenhang ($\symsub{beta}{idx_0}, \sym{beta}_1, \sym{beta}_{11}$) abgebildet werden, sind zwingend mindestens drei Stützstellen erforderlich, um das Gleichungssystem zu lösen.
Im Kontext optimaler Versuchspläne definiert nach \textcite{Pukelsheim.2006} der Maßerweiterungssatz von Carathéodory dabei eine theoretische Obergrenze für die notwendigen Stützstellen von $\sym{N}=\sym{p}(\sym{p} + 1)/2 + 1$, beziehungsweise $\sym{N}=\sym{p}(\sym{p} + 1)/2$, sofern das Interesse der Schätzung aller Elemente des Parametervektors $\sym{Beta}$ gilt \cite{Russell.2019}.
Es kann beispielsweise auf Basis der Ergebnisse des teil-faktoriellen Plans entschieden werden, ob eine Erweiterung zum voll-faktoriellen Plan überhaupt notwendig ist.
Genauso kann die Anzahl der \acp{ZP} an die beobachtete Streuung der Lebensdauerdaten angepasst werden, um eine robuste Schätzung des Fehlers zu gewährleisten.
...
Um den Grad der Abweichung von diesem Idealzustand - etwa bei asymmetrisch ausgedünnten Designs oder durch praxisbedingte Modifikationen der Versuchsraumgeometrie - quantitativ bewerten zu können, hat sich in der Literatur der \textbf{$\sym{A2}$-Indikator} als globales Maß für Nicht-Orthogonalität etabliert \cite{Box.2007, Ahn.2015}.
...
Zudem liegt \textbf{Ausgewogenheit} vor, sofern für einen jeweiligen Faktor alle anderen Faktoreinstellungen gleichmäßig aufgeteilt sind \cite{Siebertz.2026}.
...
Ein universelles Maß zur Beurteilung der Schätzgüte einzelner Parameter ist der \textbf{\ac{MSE}}. Er vereint die Varianz des Schätzers und dessen systematische Verzerrung (\textbf{Bias}) generell über alle Beobachtungen durch $1/\sym{n}\sum_{\sym{idx_i}}^{\sym{n}}\left(\symsub{y}{idx_i} - \hat{\sym{y}}_{\sym{idx_i}}\right)^2$ in einer Metrik und beschreibt für einen geschätzten Parameter $\hat{\sym{beta}}_{\sym{idx_j}}$ die erwartete quadratische Abweichung von dessen wahren Wert $\sym{beta}_{\sym{idx_j}}$:
\begin{equation}
...
Die Generierung eines solchen \ac{OMARS}-Designs erfolgt dabei nicht über geometrische Konstruktionsvorschriften (wie beim \ac{CCD}), sondern algorithmisch durch die Lösung eines ganzzahligen linearen Optimierungsproblems (engl. \textbf{\ac{ILP}}).
...
\begin{itemize}
    ...
    \item \textbf{\ac{OMARS}-Eigenschaft (Orthogonalität Haupteffekt zu Effekten zweiter Ordnung):}
...
Die Trennschärfe beschreibt als Metrik diejenige Wahrscheinlichkeit, mit der ein existierender Einfluss durch Faktoren vor dem Hintergrund des experimentellen Rauschens (vergleiche \textbf{\sym{t}-Statistik}, \textbf{\ac{SNR}}, \textbf{Signifikanztests} nach \textcite{Kleppmann.2020,Montgomery.2020,Goos.2011}) korrekt als \textbf{signifikanter} Effekt $\sym{Eff}$ identifiziert wird \cite{Siebertz.2026,Kleppmann.2020}.
...
Der Grad dieser Verschiebung wird durch den sogenannten \textbf{Nichtzentralitätsparameter} $\sym{delta_nc}$ quantifiziert \cite{Montgomery.2020,Kutner.2005}.
...
Im Kontext der Lebensdauerdatenanalyse, bei der zudem nicht-normalverteilte und zensierte Daten vorliegen, lassen sich diese Wahrscheinlichkeiten folglich ohnehin nicht mehr über klassische \textbf{\sym{t}-Tests} ableiten.
Aus der Arbeit von \textcite{Nelder.1972} kommen hier \textbf{Verallgemeinerte Lineare Modelle}, engl. \textbf{\acp{GLM}} zum Einsatz, bei denen die Signifikanz der Koeffizienten $\symsub{beta}{idx_j}$ vorzugsweise über den \textbf{\ac{LR-Test}} (oder approximativ über \textbf{Wald-Tests} - vergleiche hierzu Arbeiten von \textcite{Rigdon.2022,Meeker.2022,Montgomery.2020,McCulloch.2001}) ermittelt wird.
Da für diese komplexen Verteilungsmodelle - insbesondere unter dem Einfluss von Orthogonalitätsabweichungen und bei kleinen Stichprobenumfängen $\sym{n}$ - keine geschlossenen analytischen Lösungen zur Berechnung der Trennschärfe existieren, erfolgt die Untersuchung in der modernen Praxis numerisch mittels \textbf{\ac{MCS}} \cite{Kremer.2021,Arndt.2022,Khuri.2006}.
...
\begin{table}[htbp]
    ...
    \end{tabular}
\end{table}
Eine Übersicht verschiedener Modellierungsansätze (z.B. \textit{Arrhenius-Modell}, \textit{Inverses Potenzgesetz}, \textit{Eyring}, \textit{Coffin-Manson-Modell}, etc.) und deren Einbindung in die \ac{GLM}-Struktur findet sich in den Arbeiten von \textcite{Meeker.2022,Rigdon.2022,Modarres.2017}.
