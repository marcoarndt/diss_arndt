...
Die \textbf{\ac{RSM}} behandelt als Teildisziplin des \ac{DoE} derartige Herangehensweisen und bietet hierfür spezielle \textbf{Wirkungsflächenversuchspläne} - engl. \acp{RSD} - an.
...
Die Prädiktionsvarianz ist ein dimensionsloses Maß, das ausschließlich von der geometrischen Anordnung der \sym{N} Versuchspunkte (der Matrix $\sym{X}$) und damit positionsabhängig von einer Referenz $\symsub{x_loc}{idx_0}'=[1,\sym{x}_{01},\cdots,\sym{x}_{0\sym{k}}]$ im Parameterraum ist. 
...
{
    % Definitionen nur für diese Tabelle lokal halten
    \renewcommand{\arraystretch}{1.2} % Etwas Luft für die Formeln
    ...
    \begin{longtable}{@{}lp{0.75\textwidth}@{}}
        ...
        \caption*{\textbf{Tabelle \ref{tab:2.1_optimalitaeten} (Fortsetzung):} Übersicht der Optimalitätskriterien} \\
        \toprule
        \textbf{Kriterium} & \textbf{Zielgröße, Beschreibung und Definition} \\
        \midrule
        \endhead
  ...
  \begin{equation} \label{eq:opt_A}
    \sym{A_opt} = \min \left( \text{spur}(\sym{M}^{-1}) \right) = \min \left( \sum_{\sym{idx_j}=1}^{\sym{k}} \sym{Var}(\hat{\beta}_{\sym{idx_j}}) \right)
\end{equation}
        \compressEq
        \\ \addlinespace
        ...
        \textbf{E-Optimalität} &
        Minimierung des maximalen Eigenwerts $\sym{eigen_val}_{\max}$ der inversen Informationsmatrix. \newline
        \textit{Ziel: Worst-Case-Absicherung für den am schlechtesten geschätzten Parameter.}
        \compressEq
        \begin{equation} \label{eq:opt_E}
            \sym{E_opt} = \min \left( \sym{eigen_val}_{\max}(\sym{M}^{-1}) \right)
        \end{equation}
        \compressEq
        \\ \addlinespace
        ...
        \textbf{I-Optimalität} &
        Minimierung der durchschnittlichen Prädiktionsvarianz über den Parameterraum. \newline
        ...
        \begin{equation} \label{eq:opt_S}
            \sym{S_opt} = \max \left( \min_{\sym{idx_i} \neq \sym{idx_j}} \| \symsub{x}{idx_i} - \symsub{x}{idx_j} \| \right)
        \end{equation}
        \compressEq
        \\ \addlinespace

        \textbf{V-Optimalität} &
        Minimierung der durchschnittlichen Prädiktionsvarianz über ein diskretes Set von $\sym{N}$ Punkten. \newline
...
Die \textbf{D-Effizienz }$\sym{eff_D}$ beschreibt die Güte der Parameterschätzung im Vergleich zu einem idealen orthogonalen Design gleicher Größe. Hohe Werte (nahe $100\,\%$) stehen für eine effiziente Ausnutzung der Versuche:
\begin{equation} \label{eq:eff_D}
    \sym{eff_D} = \left( \frac{\det(\sym{M})}{\sym{n}^{\sym{p}}} \right)^{\frac{1}{\sym{p}}} = \frac{\sym{D_opt}^{\frac{1}{\sym{p}}}}{\sym{n}}.
\end{equation}
Die \textbf{G-Effizienz} $\sym{eff_G}$ bewertet die Robustheit der Vorhersage. Sie setzt die theoretisch minimal erreichbare Prädiktionsvarianz ($\sym{p}/\sym{n}$) ins Verhältnis zum tatsächlichen Worst-Case-Wert $\sym{G_opt}$ des Designs:
\begin{equation} \label{eq:eff_G}
    \sym{eff_G} = \frac{\sym{p}}{\displaystyle \max \frac{\sym{n} \sym{Var}[\hat{\sym{y}}(\sym{x})]}{\sym{sigma_sq}}}
\end{equation}\

Zuletzt muss sichergestellt werden, dass der Versuchsplan über eine ausreichende \textbf{Trennschärfe} (engl. \textbf{Power}) verfügt.
Die Trennschärfe beschreibt als Metrik diejenige Wahrscheinlichkeit, mit der ein existierender Einfluss durch Faktoren vor dem Hintergrund des experimentellen Rauschens korrekt als \textbf{signifikanter} Effekt $\sym{Eff}$ identifiziert wird.
Insbesondere bei reduzierten Versuchsplänen und unter dem Einfluss stochastischer Lebensdauerstreuung ist die Power-Analyse essenziell, um das Risiko von falsch-negativen Schlussfolgerungen zu minimieren.
Letztendlich kann die Trennschärfe auch als Garantiemetrik verstanden werden, die sicherstellt, dass ein geplanter Versuchsaufbau in der Lage ist, Effekte von praktischer Relevanz zu detektieren \colorbox{yellow}{\cite{Cohen.1988,Myers.2016}} - was  insbesondere in Bezug auf Lebensdauertests aufgrund genannter wirtschaftlicher Randbedingungen von hoher Bedeutung ist.
Formalisiert wird dies im Rahmen eines \textbf{statistischen Hypothesentests}.
...
Die Entscheidung über die Annahme oder Ablehnung von $\sym{H0}$ basiert auf dem \textbf{p-Wert}.
...
Unterschreitet der p-Wert das a priori definierte \textbf{Signifikanzniveau} $\sym{alpha}$ (üblicherweise $\sym{alpha} = 0,05$), wird $\sym{H0}$ zugunsten der Alternativhypothese $\sym{H1}$ verworfen.
...
Die Power ist definiert als das Komplement dieses Fehlers ($1 - \symsub{beta}{idx_error}$) und entspricht somit der Wahrscheinlichkeit, $\sym{H0}$ korrekterweise abzulehnen, wenn $\sym{H1}$ wahr ist.

Im Kontext der Lebensdauerdatenanalyse, bei der nicht-normalverteilte und zensierte Daten vorliegen, lassen sich diese Wahrscheinlichkeiten nicht über klassische t-Tests berechnen.
Stattdessen kommen Verfahren für \textbf{Generalisierte Lineare Modelle}, engl. \textbf{\ac{GLM}} zum Einsatz, bei denen die Signifikanz der Koeffizienten $\sym{beta}_j$ vorzugsweise über \textbf{Likelihood-Ratio-Tests} (oder approximativ über \colorbox{yellow}{Wald-Tests->Quelle}) ermittelt wird.
Da für diese komplexen Verteilungsmodelle bei kleinen Stichprobenumfängen $\sym{n}$ keine geschlossenen analytischen Lösungen zur Berechnung der Trennschärfe existieren, erfolgt die Untersuchung numerisch mittels \textbf{Monte-Carlo-Simulation}.
Hierbei wird der geplante Versuchsplan virtuell vielfach durchlaufen (z.\,B. $10.000$ Simulationsläufe).
Für jeden Lauf werden basierend auf einem angenommenen Modell Ausfallzeiten generiert, verrauscht und zensiert.
Anschließend erfolgt die Modellbildung und Hypothesenprüfung.
Der Anteil der Simulationsläufe, in denen der definierte Effekt korrekt als signifikant \colorbox{yellow}{($\text{p-Wert} < \sym{alpha}$)->p-Wert} erkannt wurde, entspricht der geschätzten Trennschärfe des Designs.
