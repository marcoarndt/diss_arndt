...
Eine visuelle Aufstellung des genannten Zusammenspiels der Parameter kann dem Parameterdiagramm, kurz \textbf{P-Diagramm}, in \colorbox{yellow}{Abbildung~P-Diagram} entnommen werden \cite{Montgomery.2020}.
...
Mittels \ac{L-DoE} können strukturiert, effizient und verbindlich Informationen gewonnen werden, die über isolierte Effekte hinausgehen und differenziert Aufschluss über \textbf{Haupteffekte} sowie etwaige \textbf{Wechselwirkungen} der Faktoren auf die Lebensdauer des Systems geben \cite{Montgomery.2020,Kleppmann.2016,Siebertz.2017,Kremer.2021}.
...
Folglich ist ein Rückgriff auf Ergebnisse aus Experimenten, die explizit für das Forschungsziel ausgelegt wurden, in dieser Phase unter Umständen noch nicht möglich.
Solche Ansätze fundieren auf einer \textbf{Systemanalyse}, die sowohl mit als auch ohne spezifisches A-priori-Wissen über das System erfolgen kann \cite{Bertsche.2022}.
Hilfsmittel zur Priorisierung einer hier zu erstellenden Parametersammlung können schließlich Entscheidungsfindungsprotokollen und Methoden aus dem Komplexitätsmanagement entsprechen, vgl. Werke von \textcite{Mayers.1997} oder \textcite{Lindemann.2008}.
Priorisierungen können sich dann entlang von Umfängen an recherchierten direkten Einflüssen, Anzahlen an plausiblen Wechselwirkungen oder schlichtweg Präferenzen aus der Testplanung ergeben.
Prozessual verorten sich Screening-Schritte zwischen der Definition des Untersuchungsziels und der Durchführung der physischen Screening-Experimente (vgl. \textit{Schritt~3} in \colorbox{yellow}{Abbildung~Doe-Steps}).
Mit deren Hilfe kann jedenfalls eine Priorisierung der Beeinträchtigungen der Systemantwort durch schiere Anzahl von rational begründbaren Einflüssen oder Wechselwirkungen auf die entsprechende Performance-Kennzahl, die Systemantwort, getroffen und somit eine Auswahl für fortführende Prozessschritte erfasst werden.
...
Werden grundsätzliche statistische Versuchspläne aus dem \ac{DoE} diskutiert, die Lebensdaueruntersuchungen aus der Praxis widerspiegeln, verstehen sich Effekte stets als Mittelwertunterschiede aus dem Vergleich zweier Einstellstufen eines Faktors.
Demzufolge nimmt der vollfaktorielle Versuchsplan auch bei Lebensdaueruntersuchungen den rationalen Standard in der Variation und Kombination von Faktorstufen ein.
