...
Der in Kapitel~\ref{chap:stand} dargelegte Stand der Forschung und Technik zur Versuchsplanungsmethodik verdeutlicht, dass sowohl die Zuverlässigkeitstechnik als auch \ac{DoE} für sich genommen etablierte Disziplinen mit jeweils umfangreichen Methodenrepertoire darstellen.
...
Ihre Stärken liegen in der sequenziellen \textit{Augmentierbarkeit}, der \textit{Rotierbarkeit} und der Fähigkeit zur unabhängigen Schätzung des reinen Fehlers (vgl. Abschnitt~\ref{subsubsec:rsm}).
Zudem lässt sich aus dem am \ac{IMA} bislang generierten Wissensstand nach \textcite{Kremer.2021} ableiten, welche methodischen Vorgehen im Rahmen des \ac{L-DoE} sich bestenfalls dazu eigenen, adäquate Schätzungen für Modellparameter \cite{Kremer.2019b} und Vertrauensbereiche \cite{Kremer.2019c} zu erhalten.
...
Ein geeigneter, \textit{effizienter} Umgang mit dieser Herausforderung ist bislang nicht etabliert.
Es existieren zwar bereits vereinzelte Ansätze zur Modifikation von \acp{CCD}, wie die Arbeiten von \textcite{Ahn.2015, Donev.2004, Box.1951} und \textcite{Ardakani.2011} zeigen.
...
Obgleich für die Anwendung im Maschinen- und Fahrzeugbau von entscheidender Bedeutung, existiert bislang kein etabliertes Vorgehen, um die Versuchspunkt-Konstellation gezielt in Prognoserichtung zu optimieren und so die Extrapolationsfähigkeit zu maximieren, ohne die Balance zwischen Modellgüte und Versuchsaufwand zu gefährden.


\subsection{Limitierungen und Effizienzbegriffe moderner Versuchsplanung}
Der Einsatz computergenerierter optimaler Versuchspläne erscheint zunächst als logischer Schritt, um maßgeschneiderte Designs für Lebensdauererprobungen zu entwerfen.
Im Kontext multivariater Lebensdauertests (\ac{ALT}) erfährt der Begriff der \textbf{Effizienz} jedoch eine signifikante Komplexitätssteigerung gegenüber der klassischen linearen Versuchsplanung, die sich in den drei Dimensionen \textit{Modellabhängigkeit}, \textit{zeitliche Dynamik} und \textit{strukturelle Flexibilität} aufgliedert.
Formal definiert sich die Effizienz eines Versuchsplans einerseits analytisch über die Informationsmatrix $\sym{M}(\sym{X}, \sym{theta})$ \cite{Khuri.2006,Stufken.2012}, andererseits generisch über das Verhältnis von Leistungsfähigkeit zur Informationsbeschaffung - ein Anspruch, der nach \textcite{Thommen.2022} in einem fundamentalen Spannungsfeld zum Grundsatz der Vollständigkeit steht.
Damit determiniert die Effizienz maßgeblich die \textbf{Effektivität} und Wirtschaftlichkeit der Lebensdauererprobung und in letzter Instanz die Verlässlichkeit der Modellierung (vgl. Abschnitt~\ref{subsec:optimal}: \sym{power}) sowie die Prognosegüte der Zuverlässigkeit $\sym{R}(\sym{t})$.

\subsubsection{Modellabhängigkeit und Lokale Optimalität}
Erstens stößt die zugrundeliegende Modellierung mittels Weibull-\acp{GLM} auf ein fundamentales Problem, das von \textcite{Khuri.2006} und \textcite{Stufken.2012} hinlänglich diskutiert wird.
...
Diese Verfahren setzen jedoch stets voraus, dass die wahren Parameterwerte $\sym{theta}$ bekannt sind oder verlässlich geschätzt werden können - beispielsweise mittels initialem \textit{best guess}.
...
Da die Information in Lebensdauertests stochastisch über die Zeit generiert wird und womöglich durch Zensierung limitiert ist, wird die Varianz der Schätzung maßgeblich durch die Anzahl der tatsächlich ausgefallenen Einheiten getrieben \cite{Nelson.2005}.\\

\textbf{Ein Versuchsplan gilt folglich nur dann als effizient, wenn er die erwartete Testdauer bei gegebener Präzision und damit unbeeinträchtigter Effektivität minimiert.}\\
...
Im direkten Vergleich zum etablierten \ac{CCD}, das durch seine geometrische Konstruktion eine starre Anzahl an Versuchen fordert, ermöglichen \ac{OMARS}-Designs zwar eine Entkopplung von Faktoren- und Versuchsanzahl (z.B. $\sym{N}=40$ statt $\sym{N} \ge 45$ für $\sym{k}=6$).
Sie resultieren jedoch in einer finalen Versuchspunktverteilung, die sich im Kontext von Aufwänden in \ac{EoL}-Testing rational nur schwer sequenziell erweitern oder anpassen lässt.
...
Anstatt sich auf die Suche nach einem theoretisch \textbf{\textit{optimalen} \ac{GLM}-Design} zu versteifen, das aufgrund der Parameterabhängigkeit in der Praxis kaum robust umsetzbar ist, erscheint die Adaption bewährter Standard-Designs (\ac{CCD}) vielversprechender.
Dabei dürfen die etablierten Optimalitätskriterien jedoch nicht als alleiniges Entwurfsziel fungieren, sondern müssen durch praxisrelevante Benchmark-Metriken ergänzt werden:
\begin{itemize}
    ...
    \item \textbf{Testdauer und Ressourceneffizienz:} Diese Metriken berücksichtigen die praktischen Rahmenbedingungen von Lebensdauertests, indem sie die erwartete Testdauer und den Ressourcenverbrauch minimieren, ohne die statistische Aussagekraft unzulässig zu beschneiden.
    \item \textbf{Trennschärfe (Power):} Sie fungiert als die entscheidende „Erfolgsgarantie“ des Tests. Insbesondere bei kostenintensiven \ac{EoL}-Versuchen ist es ökonomisch riskant, ein Design zu wählen, das zwar theoretisch optimal schätzt, aber praktisch relevante Effekte mit hoher Wahrscheinlichkeit übersieht. Die Power muss daher als primäres Entscheidungskriterium gegen Einsparpotenziale abgewogen werden.
\end{itemize}

Eine effiziente Testplanung für die multivariate Lebensdauererprobung erfordert folglich weit mehr als die Maximierung theoretischer Informationsmaße.
Sie verlangt einen strategischen Kompromiss: Das Design muss flexibel genug sein, um Extrapolationen durch \ac{ALT} gezielt zu unterstützen (Prognosegüte), robust genug, um die Unsicherheiten der nicht-linearen \ac{GLM}-Parameterschätzung abzufangen (Modellierungsgüte), und dabei eine statistische Trennschärfe gewährleisten, die das Investitionsrisiko des Versuchs rechtfertigt.
Nur wenn die Wahrscheinlichkeit der Effektdetektion (\ac{power}) in einem ökonomisch vertretbaren Verhältnis zu den Testkosten steht, ist die Bedingung der Effizienz erfüllt und Effektivität gewährleistet.

\section{Forschungsfragen und Aufbau der Arbeit} \label{sec:forschungsfragen}
Das übergeordnete Ziel dieser Arbeit, die Entwicklung effizienter multivariater Lebensdauerversuche, fußt damit auf der Prämisse, dass etablierte Standard-Designs nicht zwingend durch vollkommen neuartige Algorithmen ersetzt, sondern vielmehr intelligent adaptiert werden müssen.
Als methodischer Anker und Referenzpunkt dient hierbei das \textbf{\ac{CCD}}.
Dieser Konsens begründet sich auf einer Synthese aus verfahrenstechnischen, physikalischen und statistischen Vorzügen, die das \ac{CCD} für die Zuverlässigkeitsmodellierung im Maschinen- und Fahrzeugbau prädestinieren - wenn gleich die in dieser Arbeit vorgestellte Strategie auch generisch auf weitere \acp{RSD} übertragbar ist.
Dies lässt sich aus praktischen Erfahrungen wie beispielsweise nach \textcite{Myers.2016,Kleppmann.2020,Rigdon.2022} sowie einer Vielzahl wissenschaftlicher Publikationen ableiten (vgl. Kapitel~\ref{chap:stand}) und umfasst insbesondere folgende Aspekte:
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Bewertungsmethodik:} Wie können die Auswirkungen derartiger Anpassungen (Manipulationen der Orthogonalität) an \acp{CCD} auf die Prognosegüte quantitativ charakterisiert und vergleichbar bewertet werden?
\end{enumerate}
...
Hier wird ein Vorschlag zu einem heuristischen Screening-Ansatz vorgestellt, der darauf abzielt, die Effizienz im Prozess der Parameterselektion gezielt für Lebensdauer-beeinflussende Faktoren zu steigern und somit die Basis für nachfolgende \acp{RSD} unter dem Prädikat \textit{effizient} zu legen.
...
Zwar erfolgt die Demonstration der Methodik exemplarisch am \ac{CCD}, der Anspruch auf Gültigkeit erstreckt sich jedoch auch auf andere \acp{RSD}.
Dies begründet sich durch die in Abschnitt~\ref{subsec:optimal} dargelegten Eigenschaften, welche universell auf der Fisher-Informationsmatrix sowie der empirischen Datenlage basieren.
...
Abschließend ist in \colorbox{yellow}{\textbf{Kapitel 6}} die Vorstellung einer Fallstudie dokumentiert, um die erarbeitete Methodik an einem praxisnahen Anwendungsbeispiel zu veranschaulichen und die theoretischen Erkenntnisse in den operativen Kontext zu transferieren.
