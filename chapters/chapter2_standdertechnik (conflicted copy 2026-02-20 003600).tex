...
\subsection{Optimale Versuchsplanung und Bewertungskriterien} \label{subsec:optimal}
...
% Da für diese komplexen Verteilungsmodelle bei kleinen Stichprobenumfängen $\sym{n}$ keine geschlossenen analytischen Lösungen zur Berechnung der Trennschärfe existieren, erfolgt die Untersuchung gerne numerisch mittels \textbf{\ac{MCS}} \cite{Kremer.2021,Arndt.2022,Khuri.2006}.
...
In idealisierten, orthogonalen Standard-Versuchsplänen unter der Annahme normalverteilter Residuen lässt sich die Trennschärfe für klassische \ac{OLS}-Regressionsmodelle exakt analytisch bestimmen.
Die Grundlage hierfür bildet die Teststatistik des \textbf{t-Tests} zur Überprüfung der Signifikanz eines Koeffizienten $\symsub{beta}{idx_j}$.
Unter der Annahme der Gültigkeit der Alternativhypothese $\sym{H1}$ folgt diese Prüfgröße einer nichtzentralen t-Verteilung.
Der Grad der Verschiebung dieser Verteilung - und damit maßgeblich die resultierende Trennschärfe - wird durch den sogenannten \textbf{Nichtzentralitätsparameter} $\symsub{delta}{idx_nc}$ determiniert \cite{Montgomery.2020,Kutner.2005}.
Dieser berechnet sich in Abhängigkeit der wahren Effektstärke, der experimentellen Fehlervarianz und der geometrischen Struktur des Versuchsplans gemäß:
\begin{equation} \label{eq:nichtzentralitaet}
    \symsub{delta}{idx_nc} = \frac{|\symsub{beta}{idx_j}|}{\sym{sigma} \sqrt{\symsub{c}{idx_j\idx_j}}}.
\end{equation}
Hierbei repräsentiert $\sym{sigma}$ die Standardabweichung des Modellrauschens und $\symsub{c}{idx_j\idx_j}$ das korrespondierende $j$-te Hauptdiagonalelement der inversen Informationsmatrix $\sym{M}^{-1} = (\sym{X}'\sym{X})^{-1}$.
Die Trennschärfe ergibt sich analytisch aus der Integration der nichtzentralen Dichtefunktion über den durch das Signifikanzniveau $\sym{alpha}$ und die Freiheitsgrade $\symsub{d}{f}$ definierten kritischen Ablehnungsbereich \cite{Cohen.1988}.

In einem perfekt orthogonalen Versuchsplan ist die Informationsmatrix $\sym{M}$ strikt diagonal, wodurch keine Kovarianzen zwischen den Haupteffekten existieren.
Das Element $\symsub{c}{idx_j\idx_j}$ minimiert sich in diesem Fall auf den reziproken Wert der Quadratsumme der $j$-ten Designmatrix-Spalte, was den Nichtzentralitätsparameter und folglich die Power maximiert.
Sobald jedoch geometrische Verzerrungen auftreten und die Orthogonalität verloren geht, weisen die Nebendiagonalelemente von $\sym{M}$ Werte ungleich Null auf.
Die Invertierung zu $\sym{M}^{-1}$ führt unweigerlich zu einer Erhöhung der Diagonalelemente $\symsub{c}{idx_j\idx_j}$ (bekannt als Varianzinflation).
Gemäß Gleichung~\ref{eq:nichtzentralitaet} verringert dies den Nichtzentralitätsparameter $\symsub{delta}{idx_nc}$ unmittelbar, was den analytischen Nachweis erbringt, dass Orthogonalitätsverluste zwingend mit einem Verlust an Trennschärfe einhergehen.

Während diese Zusammenhänge für lineare Modelle und normalverteilte Daten noch geschlossen berechenbar sind, verkompliziert sich die Sachlage im Kontext der Lebensdauerdatenanalyse drastisch.
Da hier nicht-normalverteilte und zensierte Daten vorliegen, lassen sich die Wahrscheinlichkeiten ohnehin nicht mehr über klassische t-Tests ableiten.
Aus der Arbeit von \textcite{Nelder.1972} kommen hier \textbf{Verallgemeinerte Lineare Modelle}, engl. \textbf{\acp{GLM}} zum Einsatz, bei denen die Signifikanz der Koeffizienten $\sym{beta}_j$ vorzugsweise über den \textbf{\ac{LR-Test}} (oder approximativ über \textbf{Wald-Tests} - vergleiche hierzu Arbeiten von \textcite{Rigdon.2022,Meeker.2022,Montgomery.2020,McCulloch.2001}) ermittelt wird.
Da für diese komplexen Verteilungsmodelle -- insbesondere unter dem Einfluss von Orthogonalitätsabweichungen und bei kleinen Stichprobenumfängen $\sym{n}$ -- keine geschlossenen analytischen Lösungen zur exakten Berechnung der Trennschärfe existieren, erfolgt die Untersuchung in der modernen Praxis numerisch mittels \textbf{\ac{MCS}} \cite{Kremer.2021,Arndt.2022,Khuri.2006}.
