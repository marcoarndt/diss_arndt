...
\begin{figure*}[h]
    ...
    \def\svgwidth{12cm}
    \begin{normalsize}
        \import{plots/ma_abb2.02_doe_steps.pdf_tex}
    \end{normalsize}
    \includegraphics{plots/ma_abb2.02_doe_steps.pdf_tex}
    \caption{\ac{DoE}-Prozessschritte}
...
So kann beispielsweise die zufallsverteilte Lebensdauer $\sym{tau}$ in Abhängigkeit von $\sym{k} \geq 2$ technischen Beanspruchungen zunächst empirisch untersucht und anschließend modelliert sowie optimiert werden (\textit{Schritt~1} in \colorbox{yellow}{Abbildung~Doe-Steps}).
...
Screening-Schritte sind zwischen der Definition des Untersuchungsziels und der Durchführung der physischen Screening-Experimente angeordnet (vgl. \textit{Schritt~3} in \colorbox{yellow}{Abbildung~Doe-Steps} sowie Abschnitt~\ref{subsec:pläne}).
...
Sind nach Anwendung qualitativer Kreativmethoden als Auswahlverfahren im Screening nach wie vor noch derart viele Einflussfaktoren als relevante betitelt, dass daraus entsprechend Gleichung~\ref{eq:ffvp_n} ein wirtschaftlich nicht abbildbarer Versuchsumfang resultiert - in der Regel bei $\sym{k}>5$ Faktoren - ist die Screening-Strategie hin zu physikalischen Screening-Tests verschärfbar.
\textbf{Screening-Versuchspläne} wie der \textbf{teil-faktorielle Versuchsplan} (oder alternativ auch \textbf{Plackett-Burman-Versuchsplan}) eigenen sich für derartige Selektionsaufgaben \cite{Kleppmann.2016,Siebertz.2017,Montgomery.2020}.
Hier wird nur ein Anteil der voll-faktoriellen Versuchsagenda umgesetzt, sodass sich bei einem teil-faktoriellen Versuchsplan $\sym{p}$ Faktoren mehr als üblich in einer $\sym{n}=2^{\sym{k}-\sym{p}}$ umfassenden Versuchsagenda abbilden lassen.
Dabei werden zwei prinzipielle Annahmen ausgenutzt \cite{Montgomery.2021,Rigdon.2022}:
\begin{itemize}
    \item \textbf{Effekthierarchie}, die besagt, dass Effekte kleinerer Ordnung - also Haupteffekte - in der Regel größere Anteile am Gesamteffekt tragen und tendenziell signifikanter sind;
    \item \textbf{Effektvererbung}, die impliziert, dass Effekte höherer Ordnung (also quadratische Effekte oder Wechselwirkungen) tendenziell an die Signifikanz ihrer korrespondieren Haupteffekte gekoppelt sind.
\end{itemize}
Dies lässt zuletzt zu, dass Fraktionen der faktoriellen Versuchspläne entsprechend Abbildung~\ref{fig:abb2.05.1_vfdesign_plot} unter Umständen nur gewisse Effekte, beispielsweise nur Haupteffekte, als solche erkennen lassen und selbst dann nicht zweifelsfrei trennbar sind zu möglicherweise identifizierbaren Wechselwirkungen: Es liegt also eine \textbf{Vermengung}, engl. \textbf{Aliasing} der Effekte vor.
Diese bewusst in Kauf nehmend und ingenieurwissenschaftlich eingeordnet, eröffnet sich damit jedoch ein geringerer Versuchsumfang um zuletzt die effektstarken und signifikanten Faktoren aus der initialen Sammlung zu isolieren.
...
Im Sinne einer ressourceneffizienten Strategie sind darin enthaltene Versuche so konzipiert sein, dass sie direkt in einen nachfolgenden voll-faktoriellen Versuchsplan übernommen werden können.
...
\subsubsection{Optimale Versuchspläne und deren Metriken}
