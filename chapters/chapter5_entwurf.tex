...
Nur durch den Vergleich der Analyseergebnisse mit einer vorab definierten Wahrheit (engl. \textbf{Ground Truth}) lassen sich Metriken wie die Schätzpräzision und primär die \ac{power} erst bestimmen.
...
Um die Robustheit des \ac{CCD} gegenüber den eingangs erwähnten Abweichungen quantitativ zu bewerten, folgt der methodische Ansatz einem \textit{\ac{DoE} on Experimental Design}-Konzept: Die Eigenschaften der Versuchsplanung sowie der \textit{Ground Truth} werden als Faktoren eines übergeordneten Meta-Designs variiert.
Infrage kommt dabei eine aus praktischer Überlegung erfasste Auswahl an sowohl geometrische Manipulationen als auch stochastische Einflüsse sowie methodische Parameter und deren Interaktionen: die Manipulation von Sternpunkten über deren axialen Abstand sowie Co-Faktorielle Korrelation, die Anzahl der Zentralpunkte, die Anzahl der Sternpunkte, das Verhältnis von Effektstärken untereinander, das Verhältnis von System- zu Messrauschen sowie das Signifikanzniveau selbst.
...
Zur formalen Beschreibung des Untersuchungsraums werden diese acht Einflussgrößen, welche sowohl deterministische geometrische Manipulationen als auch stochastische Störgrößen abbilden, im Vektor $\sym{xi_vek} = [\symsub{xi_factor}{idx_fac_a},\dots, \symsub{xi_factor}{idx_fac_h}]'$ zusammengefasst und in Tabelle~\ref{tab:sensitivitaetsfaktoren} definiert.
Etwaige Ergänzungen wie die Variation der Anzahl der Faktoren, die Manipulation der faktoriellen Versuchspunkte oder die Einführung von Lebensdauermodellen werden bewusst ausgeschlossen, um die Komplexität des Meta-Designs handhabbar zu halten und die Interpretation der Ergebnisse zu erleichtern.
\begin{table}[htbp]
    \centering
    ...
    \end{tabular}
\end{table}
Im Gegensatz zu reinen Screening-Ansätzen kommt für dieses Meta-Design daher erneut ein \ac{CCD} mit dedizierter Blockbildung zum Einsatz.
Dies ermöglicht nicht nur die Identifikation linearer Haupteffekte, sondern auch die Analyse nicht-linearer Einflüsse der Faktoren auf die Performance-\acp{KPI} über diskrete Einstellungen wie beispielsweise ganzzahlige Umfänge von Zentralpunkten.
