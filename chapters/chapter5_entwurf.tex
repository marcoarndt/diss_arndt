...
Mithilfe von \acp{MCS} und fiktiven physikalischen Modellen wird die Entwicklung dieser Verhältnismäßigkeiten in dieser Studie systematisch aufgezeigt, siehe Abschnitt~\ref{subsec:simulationsmethodik}.
In einem zweiten Schritt werden als signifikant erfasste Einflüsse durch Design-Modifikationen hinsichtlich ihrer generischen Auswirkung auf die Modellierung bewertet (Abschnitt~\ref{subsec:sensitivitaetsanalyse}).
...
Um hierbei valide Aussagen über die \textbf{Effektivität} einer Konfiguration zu treffen, wird neben der bloßen Schätzgüte die \textbf{statistische Trennschärfe (\ac{power})} als übergeordnete Metrik herangezogen.
...
Was in der allgemeinen \ac{DoE}-Methodik oft als nachgeordnete Größe behandelt wird \cite{Rigdon.2022,Montgomery.2020}, avanciert bei kostenintensiven \ac{EoL}-Tests zum \ac{KPI}: Jede Manipulation des Designs muss zwingend gegen den potenziellen Verlust an Detektionswahrscheinlichkeit hinsichtlich der Effekte auf die Lebensdauer abgewogen werden (entsprechend Abschnitt~\ref{subsec:orthogonalitaetsabweichungen}).
...
Erst so wird quantifiziert, inwiefern zufällige oder möglicherweise bewusste Design-Modifikationen die Effizienz des Versuchsplans verändern und zu welchem Maß die Effektivität (Trennschärfe) dabei beeinträchtigt wird.
