...
Um die Robustheit des \ac{CCD} gegenüber den eingangs erwähnten Abweichungen quantitativ zu bewerten, folgt der methodische Ansatz einem \textbf{\ac{DoE} on Experimental Design}-Konzept:
...
Es muss zwischen \textbf{dispositiven Designfaktoren} (Geometrie $\symsub{xi_factor}{idx_fac_a}$, Stützstellen $\symsub{xi_factor}{idx_fac_c}, \symsub{xi_factor}{idx_fac_d}$), welche durch den Versuchsplaner aktiv modifiziert werden können, und \textbf{externen Restriktionen} (Rauschen $\symsub{xi_factor}{idx_fac_e}, \symsub{xi_factor}{idx_fac_g}$, Signifikanzniveau $\symsub{xi_factor}{idx_fac_h}$), die durch das physikalische Testsystem oder normative Vorgaben als unveränderlich zu betrachten sind, unterschieden werden.

Betrachtet man zunächst die globale \textbf{Modell-Power} ($\sym{power}_{\text{Model}}$), so zeigt sich eine extrem hohe Robustheit gegenüber allen Variationen mit einem Mittelwert von $\overline{\sym{power}} > 99,7\,\%$.
Diese pauschale Metrik verdeckt jedoch, dass die Identifikation spezifischer Einzel-Effekte -- insbesondere der für die Lebensdauerprognose kritischen quadratischen Terme -- deutlich sensitiver reagiert.
Wie Tabelle~\ref{tab:summary_power_sensitivity} zeigt, liegt die mittlere Trennschärfe für quadratische Terme ($\sym{beta}_{11}, \sym{beta}_{22}$) nur noch im Bereich von $70\,\%$ bis $79\,\%$.

Die entscheidende Erkenntnis liefert jedoch der Blick auf die \textbf{Sensitivitätsindizes} (Spalte "Dominante Sensitivität").
Für die Mehrheit der untersuchten Koeffizienten ($\sym{power}_{\text{Model}}, \sym{beta}_1, \sym{beta}_{12}, \sym{beta}_{11}$) ist nicht etwa eine Reduktion der Versuchspunkte der limitierende Faktor, sondern das \textbf{Messrauschen ($\symsub{xi_factor}{idx_fac_g}$)}.
Mit einem Sensitivitätsindex von bis zu $1,99$ (bei $\sym{beta}_{11}$) dominiert die Güte der Messtechnik die Erfolgswahrscheinlichkeit des Tests.
...
Dies birgt ein erhebliches ökonomisches Potenzial: Wenn die Trennschärfe ohnehin primär durch das Systemrauschen limitiert wird, ist zu prüfen, ob Design-Vereinfachungen (Reduktion von $\symsub{xi_factor}{idx_fac_c}$ und $\symsub{xi_factor}{idx_fac_d}$) überhaupt zu einer \textit{signifikanten} weiteren Verschlechterung führen.
Tabelle~\ref{tab:impact_cd} quantifiziert diesen "Design-Penalty".
Es zeigt sich, dass der Verzicht auf Zentralpunkte ($\symsub{xi_factor}{idx_fac_c}$) und Sternpunkte ($\symsub{xi_factor}{idx_fac_d}$) zwar die Power reduziert (z.\,B. $\Delta \sym{power} \approx -13\,\%$ für $\sym{beta}_{11}$), dieser Einfluss jedoch in der gleichen Größenordnung liegt wie die unvermeidbaren Rauscheffekte.
...
Einzig der quadratische Koeffizient $\sym{beta}_{22}$ bildet eine Ausnahme: Hier ist der geometrische Abstand der Sternpunkte ($\symsub{xi_factor}{idx_fac_a}$) mit einem extremen Index von $3,72$ der dominierende Faktor (vgl. Tabelle~\ref{tab:summary_power_sensitivity}).
Für alle anderen Terme gilt: Da die Power durch die Systemparameter ($\symsub{xi_factor}{idx_fac_e}, \symsub{xi_factor}{idx_fac_g}$) bereits maßgeblich vorbestimmt ist, besteht Spielraum zur Anpassung der Faktoren $\symsub{xi_factor}{idx_fac_c}$ und $\symsub{xi_factor}{idx_fac_d}$ in Bereiche, die aus Aspekten der Versuchszeit und des Budgets attraktiv sind, ohne die Effektivität des Tests unverhältnismäßig stärker zu kompromittieren, als es das Messsystem ohnehin tut.
